      ...
      {p.core.map((tr, i) => (
        <div key={i} className="rounded-lg border border-accent bg-accent/5 p-3">
          ...
          <p className="text-xs text-muted-foreground">
            {p.labels.applicationFrequency}: {tr.frequency} · {p.labels.appliesTo}: {tr.appliesToLabels.join(', ')}
          </p>
        </div>
      ))}

      <h3 className="text-sm font-medium">{p.labels.supporting}</h3>
      {p.supporting.map((tr, i) => (
        <div key={i} className="rounded-lg border border-border p-3">
          <p className="font-medium"><TextOrPending value={tr.name} /></p>
          <p className="text-sm text-muted-foreground">{tr.usage}</p>
          <p className="text-xs text-muted-foreground">{p.labels.applicationFrequency}: {tr.frequency}</p>
        </div>
      ))}
...
export function ReportDuration({ model }: { model: ReportModel }) {
  return (
    <section className="flex flex-col items-center gap-1 px-4 py-6 text-center">
      <h2 className="text-base font-medium">{model.titles.duration}</h2>
...
export function ReportPricing({ model }: { model: ReportModel }) {
  const perDayPending = isPending(model.pricing.perDay);
  return (
    <section className="flex flex-col gap-3 px-4 py-4">
      <h2 className="text-base font-medium">{model.titles.pricing}</h2>
      <div className="rounded-lg border border-accent p-4 text-center">
        ...
        <p className="text-xs text-muted-foreground">
          <MoneyOrPending value={model.pricing.perDay} />{!perDayPending && ` ${model.pricing.perDayLabel}`}
        </p>
      </div>
      ...
      <div className="grid gap-1">
        {model.pricing.compareAll.map((row) => (
          <div key={row.days} className="flex items-center justify-between text-xs">
            <span>{row.label}{row.isRecommended && ` · ${model.pricing.recommendedBadge}`}</span>
...
export function ReportClaims({ model }: { model: ReportModel }) {
  return (
    <section className="px-4 py-4">
      <h2 className="mb-2 text-base font-medium">{model.titles.claims}</h2>
