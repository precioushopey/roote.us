import type { MessageKey } from './en';

export const he: Record<MessageKey, string> = {
  'common.continue': 'המשך',
  'common.back': 'חזרה',
  'meta.title': 'ROOTÉ: מערכת אישית לצמיחת שיער',
  'common.next': 'הבא',
  'common.start': 'התחלה',
  'common.cancel': 'ביטול',
  'common.startOver': 'התחלה מחדש',
  'common.progressLabel': 'התקדמות',
  'common.showPassword': 'הצג סיסמה',
  'common.hidePassword': 'הסתר סיסמה',
  'common.whatThisMeans': "מה זה אומר",
  'common.hideDetails': "הסתרת הפרטים",
  'common.close': "סגירה",
  'common.readMore': "לקריאה נוספת",
  'common.beforeAfterComparison': "השוואה: {before} / {after}",
  'common.brandOf': "מותג של",
  'common.packagingAlt': "אריזת {name}",
  'notFound.policy.title': "מדיניות",
  'notFound.policy.body': "לא הצלחנו למצוא את דף המדיניות הזה.",
  'notFound.product.title': "מוצר",
  'notFound.product.body': "לא הצלחנו למצוא את המוצר הזה.",
  'notFound.solution.title': "פתרון",
  'notFound.solution.body': "לא הצלחנו למצוא את דף הפתרון הזה.",
  'brand.tagline': 'מערכת אישית לצמיחת שיער',
  'nav.language.open': 'שינוי שפה',
  'nav.language.title': 'שפה',
  'nav.language.current': 'שפה נוכחית: {name}',

  'scale.norwood.label': 'סולם נורווד–המילטון',
  'scale.ludwig.label': 'סולם לודוויג',

  'severity.mild': 'מוקדם',
  'severity.moderate': 'בינוני',
  'severity.established': 'מבוסס',

  'zone.frontal-hairline': 'קו השיער הקדמי',
  'zone.temples': 'הרקות',
  'zone.mid-scalp': 'מרכז הקרקפת',
  'zone.crown-vertex': 'קודקוד הראש',

  // TODO: confirm with client — clinical Hebrew phrasing (spec §12 PENDING inventory)
  'zone-note.frontal-hairline': 'נסיגה לאורך קו השיער הקדמי.',
  'zone-note.temples': 'שתי פינות הרקות נסוגו לאחור.',
  'zone-note.mid-scalp': 'כיסוי מופחת במרכז הקרקפת.',
  'zone-note.crown-vertex': 'דילול בקודקוד עם שקיפות של הקרקפת.',

  'metric.pattern-stage': 'שלב הדפוס',
  'metric.relative-density': 'צפיפות יחסית',
  'metric.thickness-caliber': 'עובי השערה',
  'metric.scalp-visibility': 'חשיפת הקרקפת',

  'note.treatment-naive': 'לא בוצע טיפול קודם לנשירת שיער.',
  'note.prior-no-response': 'טיפול קודם ללא שיפור מורגש.',
  'note.prior-partial': 'טיפול קודם עם שיפור חלקי.',
  'note.family-history-positive': 'קיים רקע משפחתי של נשירת שיער.',
  'note.family-history-unknown': 'רקע משפחתי לא ודאי.',
  'note.family-history-negative': 'אין רקע משפחתי מדווח של נשירת שיער.',

  'summary.norwood.mild': 'דילול מוקדם בדפוס, ממוקד באזורים בודדים. איכות השערה נראית תקינה.',
  'summary.norwood.moderate': 'דילול בדפוס שכבר מבוסס באזורים המסומנים.',
  'summary.norwood.established': 'דילול ותיק בדפוס עם קרקפת חשופה במספר אזורים.',
  'summary.ludwig.mild': 'דילול מפוזר מוקדם, בולט בעיקר לאורך הפסוקת.',
  'summary.ludwig.moderate': 'דילול מפוזר עם התרחבות הפסוקת וירידה בנפח.',
  'summary.ludwig.established': 'דילול מפוזר ניכר במרכז הקרקפת.',



  'diagnosis.gender.title': 'מה המין שלך?',
  'diagnosis.gender.male': 'גבר',
  'diagnosis.gender.female': 'אישה',

  'photo.angle.front': 'קדמי',
  'photo.angle.top': 'עליון',
  'photo.angle.crown': 'קודקוד',
  'photo.angle.hairline': 'קו שיער',
  'photo.add': 'הוספת תמונה',
  'photo.retake': 'צילום מחדש',
  'photo.uploading': 'מעבד…',
  'common.remove': 'הסרה',
  'photo.error.type': 'יש לבחור קובץ תמונה.',
  'photo.error.size': 'התמונה גדולה מדי (עד 15MB).',
  'photo.error.generic': 'לא ניתן לעבד את התמונה. נסו אחרת.',

  'diagnosis.photos.title': 'הוספת תמונות של האזור',
  'diagnosis.photos.howto': 'תאורה טובה, שיער יבש, מצלמה ישרה. קדמי, עליון, קודקוד, קו שיער.',
  'diagnosis.photos.privacy': 'התמונות משמשות אך ורק לאבחון שלך.',
  'diagnosis.photos.needAll': 'הוספת {have} מתוך {total} תמונות. יש להוסיף את כל ארבע הזוויות כדי להמשיך.',

  'q.counter': 'שאלה {index} מתוך {total}',

  'analysis.title': 'מנתחים את השיער שלך',

  'ready.title': 'אבחון השיער שלך הושלם בהצלחה',
  'ready.teaser': '{scale} · דפוס {severity} · {zones} אזורים מסומנים',
  'ready.email.placeholder': 'כתובת אימייל',
  'ready.email.submit': 'לצפייה בתוצאות האישיות שלי',
  'ready.email.invalid': 'נא להזין כתובת אימייל תקינה.',
  'ready.consent': 'התוצאות ייפתחו במסך הבא. ייתכן שנפנה אליך במייל בנוגע לתוכנית.',

  'report.header.title': 'דוח שיער אישי',
  'report.header.reportIdLabel': 'מזהה דוח',
  'report.header.generatedLabel': 'הופק בתאריך',
  'report.section.photos.title': 'התמונות שלך',
  'report.section.analysis.title': 'ניתוח AI',
  'report.section.hairLossType.title': 'סוג נשירת השיער',
  'report.section.currentSituation.title': 'המצב הנוכחי',
  'report.section.plan.title': 'תוכנית הטיפול האישית שלך',
  'report.section.plan.matchedBadge': 'מותאם לסריקה שלך',
  'report.section.duration.title': 'משך התוכנית המומלץ',
  'report.section.pricing.title': 'מחיר',
  'report.section.claims.title': 'יעילות וציפיות',
  'report.section.observations': 'תצפיות עיקריות',
  'report.section.productsIncluded': 'המוצרים הכלולים',
  'report.section.safety': 'בטיחות והתאמה',
  'report.gray.title': 'פרופיל אפור',
  'report.gray.stage': 'שלב',
  'report.gray.area': 'בולט ביותר',
  'report.gray.pace': 'התקדמות',
  'report.gray.routineBody': 'השגרה לשיער אפור משלבת תוסף יומי עם סרום מקומי, בשימוש עקבי. אין בכך טענה להאטה או להיפוך של ההאפרה.',
  'report.safety.reviewRequired': 'התוכנית הזו כוללת רכיב בעוצמת מרשם. ההתאמה וההנחיות נקבעות בבדיקת טיפול לפני כל שילוח.',
  'report.safety.notRequired': 'התוכנית הזו מורכבת ממוצרי תמיכה ואינה מחייבת בדיקת טיפול.',
  'report.plan.review.requiresReview': 'המטרה שלך דורשת מעט מידע נוסף לפני שנוכל להמליץ על התוכנית המתאימה. תוצאות הניתוח שלך למטה מוכנות, ונחזור אליך בנוגע להמלצת המוצר.',
  'report.plan.review.professional': 'בהתאם למה ששיתפת, אנו ממליצים על הערכה מקצועית לפני תחילת כל טיפול. עצרנו את המלצת המוצר האוטומטית, ותוצאות הניתוח שלך למטה עדיין זמינות לצפייה.',
  'report.plan.review.pendingApproval': 'תוכנית טיפול צמיחת השיער שלך הותאמה לדפוס שלך וממתינה לסיום בדיקה קלינית לפני שנוכל להציג אותה כאן. תוצאות הניתוח שלך למטה מוכנות בינתיים.',
  'recommend.rationale.thickerFuller': 'הותאם למטרה שלך של שיער סמיך ומלא יותר.',
  'recommend.rationale.thickerFullerReview': 'למטרת שיער סמיך ומלא יותר נדרש דפוס ידוע, לכן עצרנו את ההמלצה האוטומטית.',
  'recommend.rationale.thickerFullerPending': 'עוצמת הטיפול לצפיפות ולמלאות הותאמה לדפוס שלך וממתינה לאישור קליני לפני ההפעלה.',
  'recommend.rationale.thickerFullerActive': 'הותאם לעוצמת הטיפול של הדפוס שלך לשיער סמיך ומלא יותר.',
  'recommend.rationale.slowGraying': 'הותאם למטרה שלך של האטת הזדקנות שיער נוספת.',
  'recommend.rationale.stopLoss': 'הותאם למטרה שלך של עצירת נשירת שיער מתמשכת.',
  'recommend.rationale.hairGrowthReview': 'טיפול לצמיחת שיער דורש דפוס ידוע, לכן עצרנו את ההמלצה האוטומטית.',
  'recommend.rationale.hairGrowthPending': 'עוצמת טיפול צמיחת השיער שלך הותאמה לדפוס שלך וממתינה לאישור קליני לפני הפעלה.',
  'recommend.rationale.hairGrowthActive': 'הותאם לעוצמת טיפול צמיחת השיער של הדפוס שלך.',
  'recommend.rationale.other': 'המטרה שלך דורשת מעט מידע נוסף לפני שנוכל להמליץ על תוכנית.',
  'recommend.rationale.fallback': 'לא הצלחנו לקבוע המלצה מהתשובות שלך.',
  'report.hairLossType.typeLabel': 'דפוס {band}',
  'report.cta.label': 'המשך לתשלום',
  'report.pdf.title': 'דוח שיער אישי של ROOTÉ',
  'report.pdf.download': 'הורדת PDF',
  'report.duration.label': '{days} ימים',
  'report.duration.rationale': 'דפוס {severity}, מטרת {emphasis}',
  'report.emphasis.stabilize': 'ייצוב',
  'report.emphasis.regrow': 'צמיחה מחדש',
  'report.emphasis.stabilize-regrow': 'ייצוב וצמיחה מחדש',
  'report.pricing.perDayLabel': 'ליום',
  'report.pricing.compareTitle': 'השוואת כל אורכי התוכנית',
  'report.pricing.recommendedBadge': 'מומלץ',
  'report.pricing.totalLabel': 'סה"כ',
  'report.pricing.features':
    'מעקב התקדמות עם תמונות|תזכורות לכל שלב|תמיכה מתמשכת מצוות הטיפול שלך|התאמת התוכנית שלך תוך כדי התהליך',
  'report.claims.effectiveness.label': 'יעילות',
  'report.claims.timeToVisibleResults.label': 'זמן טיפוסי לתוצאות נראות לעין',
  'report.claims.doctorFollowUpCost.label': 'עלות מעקב רפואי',
  'report.plan.core.title': 'טיפול עיקרי',
  'report.plan.supporting.title': 'טיפול תומך',
  'report.plan.applicationFrequencyLabel': 'תדירות יישום',
  'report.plan.durationLabel': 'משך',
  'report.plan.appliesToLabel': 'חל על',
  'report.currentSituation.nextStep': 'זו נקודת התחלה. התוכנית האישית שלך למטה בנויה סביב מה שזוהה בסריקה שלך.',
  'report.notFound.title': 'הדוח לא נמצא',
  'report.notFound.body': 'קישור הדוח פג תוקף, או שיש להתחיל את האבחון מחדש.',
  'report.notFound.cta': 'התחל אבחון חדש',
  'report.emailPreview.subject': 'דוח ניתוח השיער שלך מבית ROOTÉ',
  'report.emailPreview.intro': 'הדוח האישי שלך מוכן.',
  'report.emailPreview.openOnSite': 'לצפייה בדוח המלא',
  'report.ribbon': 'מבוסס רפואת עור',
  'report.section.cover.title': 'התוכנית שלך',
  'report.section.scan.title': 'הסריקה שלך',
  'report.section.program.title': 'התוכנית שלך',
  'report.intro.greeting': 'התוכנית האישית שלך מוכנה.',
  'report.intro.body':
    'נבנתה מהסריקה שלך: הדפוס שלך, האזורים שסומנו, והמקום שבו השיער שלך נמצא במחזור הצמיחה. למטה מופיע הטיפול שמותאם לכך, ומה לצפות לאורך הדרך.',
  'report.regimen.title': 'הטיפול שלך',
  'report.regimen.howToApply': 'למרוח {frequency}',
  'report.regimen.howToUse': 'לשימוש {frequency}',
  'report.regimen.badges': 'ללא בושם|מבוסס רפואת עור|שגרה אחת, לא ארון מלא',
  'report.treatment.roote-topical.form': 'תרחיף משולב אחד',
  'report.treatment.roote-topical.addresses': 'צמיחה מחדש|מסלול ה-DHT|אזורים מושפעים',
  'report.treatment.roote-topical.mechanism1':
    'DHT ממזער בהדרגה זקיקים רגישים ומקצר כל מחזור צמיחה. התרחיף שלך משלב ממריץ צמיחה עם רכיבים פעילים ממוקדי-DHT כדי לפעול על שניהם במקביל.',
  'report.treatment.roote-topical.mechanism2':
    'הוא נמרח רק היכן שהסריקה שלך סימנה דילול, בעוצמה שרמת החומרה שלך דורשת. פורמולה אחת, לא בקבוקים נפרדים לשכבות.',
  'report.treatment.derma-stim.form': 'שגרת קרקפת שבועית',
  'report.treatment.derma-stim.addresses': 'זרימת דם|תמיכה בזקיק',
  'report.treatment.derma-stim.mechanism1':
    'שגרת קרקפת קצרה ומודרכת פעם בשבוע שתומכת בזרימת הדם לזקיק לצד התרחיף.',
  'report.treatment.cleanser.form': 'תכשיר ניקוי יומי',
  'report.treatment.cleanser.addresses': 'מצב הקרקפת|שמירת הטיפול',
  'report.treatment.cleanser.mechanism1':
    'תכשיר ניקוי יומי עדין שמפותח כדי לא לשטוף את הטיפול המקומי או לייבש את הקרקפת.',
  'report.treatment.density-serum.form': 'סרום יומי ללא שטיפה',
  'report.treatment.density-serum.addresses': 'מלאות נראית לעין|עובי הסיב',
  'report.treatment.density-serum.mechanism1':
    'סרום ללא שטיפה שמפותח לתמיכה בצפיפות נראית לעין ובמראה מלא יותר, מיושם באזורים שהסריקה שלך סימנה.',
  'report.treatment.density-6.form': 'טיפול מקומי יומי',
  'report.treatment.density-6.addresses': 'צמיחת שיער|אזורים מושפעים',
  'report.treatment.density-6.mechanism1':
    'טיפול לצמיחת שיער המותאם לדפוס, מיושם בעוצמה שההערכה שלך קוראת לה.',
  'report.treatment.density-10.form': 'טיפול מקומי יומי',
  'report.treatment.density-10.addresses': 'צמיחת שיער|אזורים מושפעים',
  'report.treatment.density-10.mechanism1':
    'טיפול לצמיחת שיער המותאם לדפוס, מיושם בעוצמה שההערכה שלך קוראת לה.',
  'report.treatment.density-15.form': 'טיפול מקומי יומי',
  'report.treatment.density-15.addresses': 'צמיחת שיער|אזורים מושפעים',
  'report.treatment.density-15.mechanism1':
    'טיפול לצמיחת שיער המותאם לדפוס, מיושם בעוצמה שההערכה שלך קוראת לה.',
  'report.treatment.regrowth-shampoo.form': 'שמפו ליום השטיפה',
  'report.treatment.regrowth-shampoo.addresses': 'נשירה מתמשכת|מצב הקרקפת',
  'report.treatment.regrowth-shampoo.mechanism1':
    'שמפו שמפותח לסייע בהפחתת נשירה מתמשכת כחלק משגרת השטיפה הרגילה שלך.',
  'report.treatment.gray-support.form': 'קפסולה יומית',
  'report.treatment.gray-support.addresses': 'תמיכה בפיגמנטציה',
  'report.treatment.gray-support.mechanism1':
    'קפסולה יומית שמפותחת לתמיכה בפיגמנטציית השיער ולהאטת הזדקנות נוספת.',
  'report.treatment.gray-serum.form': 'סרום יומי ללא שטיפה',
  'report.treatment.gray-serum.addresses': 'תמיכה בפיגמנטציה',
  'report.treatment.gray-serum.mechanism1':
    'סרום מקומי משלים לאזורים שבהם ההזדקנות בולטת ביותר.',
  'report.actives.title': 'הרכיבים הפעילים בפורמולה שלך',
  'report.actives.note': 'כל רכיב פעיל מתועד למנגנון פעולה מפורסם.',
  'report.expect.title': 'מה לצפות',
  'report.expect.intro':
    'אנחנו מפרסמים נתוני מחקר אמיתיים עבור הפורמולה הזו ברגע שהם זמינים, לעולם לא מספר placeholder.',
  'report.faq.title': 'שאלות נפוצות',
  'level.low': 'נמוך',
  'level.medium': 'בינוני',
  'level.high': 'גבוה',
  'role.regrowth-stimulant': 'ממריץ צמיחה מחדש',
  'role.dht-blocker': 'חוסם DHT',
  'role.dht-support': 'תמיכה בחסימת DHT',
  'role.proprietary-support': 'רכיב תומך ייחודי',
  'usage.apply-scalp-affected': 'למרוח על אזורי הקרקפת שזוהו בסריקה שלך',
  'frequency.twice-daily': 'פעמיים ביום',
  'usage.derma-stim': 'הנחיות רשות לטיפוח הקרקפת: לא טיפול, ולא הבטחה לצמיחת שיער',
  'frequency.weekly': 'שבועי',
  'usage.cleanse': 'להשתמש במקום שמפו רגיל',
  'frequency.daily': 'יומי',
  'frequency.daily-evening': 'יומי, בערב',
  'frequency.daily-morning': 'יומי, בבוקר',
  'frequency.wash-day': 'בימי חפיפה',
  'usage.gray-support': 'ליטול עם ארוחה, לפי ההנחיות',
  'usage.gray-serum': 'למרוח על אזורי הקרקפת הרלוונטיים',

  'start.rail.plan': 'תוכנית',
  'start.rail.payment': 'תשלום',
  'start.noReport.body': 'עליך להשלים ניתוח שיער לפני יצירת התוכנית שלך.',
  'start.noReport.devSeedCta': 'טעינת דוח הדגמה (למפתחים בלבד)',
  'start.account.emailLabel': 'אימייל',
  'start.account.passwordLabel': 'סיסמה',
  'start.account.submit': 'יצירת חשבון',
  'start.account.haveAccount': 'כבר יש לך חשבון?',
  'start.account.signInCta': 'כניסה',
  'auth.signUp.title': 'הרשמה',
  'auth.login.title': 'כניסה',
  'auth.login.submit': 'כניסה',
  'auth.login.noAccount': 'עדיין אין לך חשבון?',
  'auth.login.startCta': 'התחילו אבחון חינם',
  'auth.login.error.notFound': 'לא נמצא חשבון עבור כתובת האימייל הזו.',
  'auth.login.error.wrongPassword': 'הסיסמה אינה תואמת.',
  'auth.login.withOrder': "כניסה עם מספר ההזמנה",
  'auth.login.withPassword': "כניסה עם סיסמה",
  'auth.login.orderLabel': "מספר הזמנה",
  'auth.login.orderHint': "המספר מופיע באישור ההזמנה שלך.",
  'auth.login.error.orderNotFound': "לא מצאנו הזמנה עם המספר והאימייל האלה.",
  'auth.login.error.noPassword': "לחשבון הזה עדיין אין סיסמה. אפשר להיכנס עם מספר ההזמנה.",
  'auth.signUp.confirmLabel': "אימות סיסמה",
  'auth.signUp.error.mismatch': "הסיסמאות אינן תואמות.",
  'start.account.error.invalidEmail': 'נא להזין כתובת אימייל תקינה.',
  'start.account.error.weakPassword': 'יש להשתמש בלפחות 8 תווים.',
  'start.account.error.duplicateEmail': 'קיים כבר חשבון עם כתובת אימייל זו.',
  'start.plan.durationLegend': 'משך התוכנית',
  'start.plan.recommendedBadge': 'מומלץ עבורך',
  'start.plan.continue': 'המשך לתשלום',
  'start.checkout.summaryTitle': 'סיכום הזמנה',
  'start.checkout.change': 'שינוי',
  'start.checkout.shipping': 'משלוח',
  'start.checkout.shippingFree': 'משלוח רגיל חינם',
  'start.checkout.total': 'סה"כ',
  'start.checkout.error.payment': 'התשלום נכשל. נא לנסות שוב.',

  // --- משפך התוכנית (WP7) ---
  'program.plan.priceNote': 'מחירים, חיסכון והחיוב המדויק מאושרים בקופה.',
  'program.checkout.programTitle': 'תוכנית ROOTÉ שלך',
  'program.checkout.recommendedFor': 'מומלץ עבור',
  'program.checkout.includes': 'כולל',
  'program.checkout.packaging': 'אריזה',
  'program.checkout.packMen': 'שחור',
  'program.checkout.packWomen': 'ענבר',
  'program.checkout.review': 'בדיקת טיפול',
  'program.checkout.reviewRequired': 'נדרשת',
  'program.checkout.reviewNotRequired': 'לא נדרשת',
  'program.checkout.subscriptionOptIn': 'להגדיר את התוכנית להזמנה חוזרת אוטומטית',
  'program.checkout.subscriptionBody': 'אופציונלי. אם נדרשת בדיקת טיפול, הזמנה חוזרת מתחילה רק לאחר שנמצאת מתאים/ה. ניתן להשהות או לבטל בכל עת מהחשבון.',
  'program.checkout.noSurprise': 'אין חיוב מתחדש שנבחר מראש. תחויב/י רק על מה שמוצג למעלה, אלא אם תבחר/י להצטרף.',
  'program.checkout.legal': 'החיוב המדויק, תדירות המשלוח, תנאי החידוש, מסים, משלוח ואופן הביטול מוצגים לפני האישור וגם בתנאי המנוי.',

  // טופס התשלום המשותף — בשימוש /program/checkout ו-/bag/checkout דרך <CheckoutFields>.
  'checkout.contactTitle': "פרטי קשר",
  'checkout.firstName': "שם פרטי",
  'checkout.lastName': "שם משפחה",
  'checkout.email': 'אימייל',
  'checkout.mobile': "נייד",
  'checkout.mobileHint': "לעדכוני משלוח בלבד.",
  'checkout.city': 'עיר',
  'checkout.postal': 'מיקוד',
  'checkout.state': "מדינה/מחוז",
  'checkout.country': "מדינה",
  'checkout.countryUS': "ארצות הברית",
  'checkout.addressLine1': "שורת כתובת 1",
  'checkout.addressLine2': "שורת כתובת 2",
  'checkout.billingTitle': "כתובת לחיוב",
  'checkout.shippingTitle': "כתובת למשלוח",
  'checkout.shippingSameAsBilling': "כתובת המשלוח זהה לכתובת לחיוב",
  'checkout.paymentTitle': 'תשלום',
  'checkout.testNotice': 'ממשק בדיקה: לא מתבצע תשלום אמיתי. אין להזין כרטיס אמיתי.',
  'checkout.cardName': 'שם בעל הכרטיס',
  'checkout.cardNumber': 'מספר כרטיס',
  'checkout.expiry': 'תוקף',
  'checkout.cvc': 'CVC',
  'checkout.altPayment': 'PayPal / ביט / Apple Pay (בקרוב)',
  'checkout.paymentMethodTitle': "אמצעי תשלום",
  'checkout.paymentMethod.card': "כרטיס אשראי",
  'checkout.submit': 'ביצוע הזמנה',
  'checkout.submitting': 'מבצע הזמנה…',
  'checkout.termsAgree': 'ביצוע ההזמנה מהווה הסכמה ל',
  'checkout.error.card': 'נא להזין מספר כרטיס תקין.',
  'checkout.error.expiry': 'נא להזין תוקף בפורמט MM/YY.',
  'checkout.error.cvc': 'נא להזין CVC תקין.',
  'checkout.error.required': "שדה זה הוא חובה.",
  'checkout.error.email': "נא להזין כתובת אימייל תקינה.",
  'checkout.error.mobile': "נא להזין מספר נייד אמריקאי תקין.",
  'checkout.error.postal': "נא להזין מיקוד בן 5 ספרות.",
  'start.success.title': 'הכל מוכן',
  'start.success.saveOrderNote': 'צלמו מסך של המסך הזה או העתיקו את מספר ההזמנה כדי שתוכלו להתחבר מאוחר יותר.',
  'start.success.copyOrderId': 'העתקת מספר ההזמנה',
  'start.success.copied': 'הועתק',
  'start.success.point1': 'התוכנית שלך מתחילה היום.',
  'start.success.point2': 'נזכיר לך מתי ליישם כל שלב.',
  'start.success.point3': 'עקבו אחרי ההתקדמות שלכם עם תמונות תוך כדי.',
  'start.success.cta': 'למעבר לתוכנית שלי',
  'start.success.stepsTitle': 'מה קורה עכשיו',
  'start.success.step1.title': 'הערכה שלך יוצאת לדרך',
  'start.success.step1.body': 'המוצרים שלך מוכנים ובדרך אליך.',
  'start.success.step2.title': 'אישור קבלה',
  'start.success.step2.body': 'כשהחבילה תגיע, אשר/י זאת בחשבון שלך כדי להתחיל את היום הראשון שלך.',
  'start.success.step3.title': 'עקוב/י אחרי השגרה שלך',
  'start.success.step3.body': 'נשלח לך תזכורת לכל שלב.',
  'start.success.step4.title': 'עקוב/י אחרי ההתקדמות שלך',
  'start.success.step4.body': 'הוסף/הוסיפי תמונות תוך כדי התהליך כדי לראות שינוי לאורך זמן.',

  'marketing.nav.howItWorks': 'תהליך',
  'marketing.nav.solutions': 'פתרונות',
  'marketing.nav.science': 'המדע',
  'marketing.nav.products': 'המוצרים',
  'marketing.nav.system': 'השיטה שלנו',
  'marketing.nav.about': 'אודות',
  'marketing.nav.hairScan': 'HairHealth.ai',
  'marketing.nav.home': 'בית',
  'marketing.nav.magazine': 'מגזין',
  'marketing.nav.aiSection': 'הבינה המלאכותית',
  'marketing.nav.more': 'עוד',
  'marketing.nav.faq': 'שאלות נפוצות',
  'marketing.nav.blog': 'בלוג',
  'marketing.nav.support': 'תמיכה',
  'marketing.nav.shop': 'חנות',
  'marketing.nav.account': 'חשבון',
  'marketing.nav.bag': 'הסל',
  'marketing.nav.cta': 'להתחלת אבחון שיער חינם',
  'marketing.nav.secondaryCta': 'איך זה עובד',
  'marketing.nav.openMenu': 'פתיחת תפריט',
  'marketing.nav.closeMenu': 'סגירת תפריט',

  'marketing.magazine.eyebrow': 'המדע שמאחורי המערכת',
  'marketing.magazine.title': 'הבינו מה באמת נמצא בשגרה שלכם.',
  'marketing.magazine.body':
    'מבט מקרוב על הסיבה לנשירת שיער, על תפקידו של כל רכיב, ועל האופן שבו כל טיפול של ROOTÉ משתלב בתוכנית שלכם.',
  'marketing.magazine.heroMediaAlt': 'רכיבים צמחיים וכלי זכוכית מעבדתיים, פורמולות פעילות בתאורת סטודיו',
  'marketing.magazine.whyEyebrow': 'היסודות',
  'marketing.magazine.whyHeading': 'למה שיער נושר',
  'marketing.magazine.whyMediaAlt': 'תקריב של קו שיער דליל, ללא פנים',
  'marketing.magazine.whyCta': 'גלו מה מטפל בזה',
  'marketing.magazine.timelineHeading': 'איך תוצאות מתפתחות בדרך כלל',
  'marketing.magazine.timelinePendingLabel': 'לוח זמנים לתוצאות',
  'marketing.magazine.ingredientsEyebrow': 'רכיב אחר רכיב',
  'marketing.magazine.ingredientsHeading': 'ספריית הרכיבים',
  'marketing.magazine.formatsEyebrow': 'כמוסות, סרומים ותמיסות',
  'marketing.magazine.formatsHeading': 'למה טיפולים מגיעים בצורות שונות',
  'marketing.magazine.productsEyebrow': 'המערך המלא',
  'marketing.magazine.productsHeading': 'כל טיפול של ROOTÉ',
  'marketing.magazine.ctaHeading': 'מוכנים למצוא את התוכנית שלכם?',
  'marketing.magazine.ctaBody': 'גלו איך הרכיבים האלה מתאימים לשיער שלכם, עם ניתוח אישי וחינמי.',
  'marketing.magazine.formatLabel.topicalSolution': 'תמיסות מקומיות',
  'marketing.magazine.formatLabel.capsuleSupplement': 'תוספי קפסולות',
  'marketing.magazine.formatLabel.serum': 'סרומים',
  'marketing.magazine.formatLabel.shampoo': 'שמפו',

  'marketing.footer.explore': 'לחקור',
  'marketing.footer.solutions': 'פתרונות',
  'marketing.footer.company': 'החברה',
  'marketing.footer.legal': 'משפטי',
  'marketing.footer.customerSupport': 'שירות לקוחות',
  'marketing.footer.cta': 'להתחלת אבחון חינם',
  'marketing.footer.account': 'החשבון שלך',
  'marketing.footer.getStarted': 'להתחיל',
  'marketing.footer.terms': 'תנאים',
  'marketing.footer.privacy': 'פרטיות',
  'marketing.footer.rights': 'כל הזכויות שמורות.',
  'marketing.popup.eyebrow': 'לא בטוח/ה מאיפה להתחיל?',
  'marketing.popup.title': 'למצוא את תוכנית ROOTÉ שלך',
  'marketing.popup.body': 'אבחון שיער מודרך וחינמי. כמה דקות, ללא התחייבות.',
  'marketing.popup.cta': 'להתחלת אבחון חינם',
  'marketing.popup.continueCta': 'להמשך האבחון',
  'marketing.popup.dismiss': 'לא עכשיו',

  'marketing.cta.default.title': 'התוכנית שלך מתחילה באבחון אחד חינם.',
  'marketing.cta.default.body':
    'עונים על כמה שאלות, מוסיפים תמונת קרקפת, ומקבלים תוכנית שמותאמת לדפוס שלך.',
  'marketing.cta.default.item1': 'קבלו את התוכנית האישית שלכם תוך דקות',
  'marketing.cta.default.item2': 'נבחן מול הנחיות מבוססות-רפואת עור',
  'marketing.cta.default.item3': 'עקבו אחרי ההתקדמות שלכם עם השוואות תמונות',

  'marketing.a11y.skipToContent': 'דילוג לתוכן',
  'marketing.nav.menuLabel': 'תפריט',
  'marketing.nav.primaryLabel': 'ניווט ראשי',

  'marketing.home.hero.eyebrow': 'מערכת אישית לצמיחת שיער',
  'marketing.home.hero.support':
    'ענו על שאלון קצר וצרפו תמונה של הקרקפת לקבלת אבחון שיער חינם בסיוע בינה מלאכותית ותוכנית טיפול מותאמת אישית.',
  'marketing.home.hero.mediaAlt':
    'בקבוקי ROOTÉ ברמות 6, 10 ו-15 באריזות שחורות וענבר, עומדים על בסיסי אבן',

  'marketing.home.strip.item1': 'אבחון שיער בסיוע בינה מלאכותית',
  'marketing.home.strip.item2': 'תוכנית אישית',
  'marketing.home.strip.item3': 'מעקב התקדמות',
  'marketing.home.strip.item4': 'מסע טיפול מתמשך',

  'marketing.home.concern.eyebrow': 'בעיות שיער',
  'marketing.home.concern.heading': 'מה תרצה/י להבין?',
  'marketing.home.concern.cta': 'לקריאת המגזין',
  'marketing.home.concern.thinningMediaAlt': 'תקריב של קו שיער דליל, ללא פנים',
  'marketing.home.concern.grayMediaAlt': 'מאקרו של שערות אפורות טבעיות',
  'marketing.home.concern.bothMediaAlt': 'תקריב של הקודקוד עם דלילות ושיער אפור יחד',

  'marketing.home.how.eyebrow': 'איך ROOTÉ עובד',
  'marketing.home.how.heading': 'מסע טיפול שנבנה סביב השיער שלך.',
  'marketing.home.how.mediaAlt': 'רצף של חמישה שלבים: לנתח, להבין, להתאים, לטפל, לעקוב',
  'marketing.home.how.step5MediaAlt': 'השוואת סריקת קרקפת מבדיקת הבסיס לבדיקה מאוחרת יותר',

  'marketing.home.analysis.eyebrow': 'אבחון שיער של ROOTÉ',
  'marketing.home.analysis.heading': 'נקודת ההתחלה שלך היא הנתונים שלך.',
  'marketing.home.analysis.body': 'מעלים תמונות שיער מודרכות וממלאים הערכה קצרה בזמן שהסריקה מעובדת.',
  'marketing.home.analysis.cardTitle': 'פרופיל שיער לדוגמה',
  'marketing.home.analysis.rowDensity': 'צפיפות שיער',
  'marketing.home.analysis.rowPattern': 'דפוס',
  'marketing.home.analysis.rowProgression': 'התקדמות',
  'marketing.home.analysis.disclaimer':
    'להמחשה בלבד. נתונים אמיתיים מגיעים מהאבחון שלך לאחר חיבור שירות ניתוח.',
  'marketing.home.analysis.mediaAlt': 'טלפון המציג את ממשק סריקת הקרקפת',

  'marketing.home.system.eyebrow': 'מערכת אישית',
  'marketing.home.system.heading': 'לא מוצר אחד לכולם.',
  'marketing.home.system.body':
    'תוכנית ROOTÉ שלך יכולה לשלב טיפולים ממוקדים ומוצרי תמיכה לפי ההערכה וההתאמה שלך.',
  'marketing.home.system.men': 'מערכת לגברים',
  'marketing.home.system.women': 'מערכת לנשים',
  'marketing.home.system.menMediaAlt': 'מערכת ROOTÉ לגברים באריזה שחורה',
  'marketing.home.system.womenMediaAlt': 'מערכת ROOTÉ לנשים באריזת ענבר',
  'marketing.home.system.cta': 'חנות מוצרים',
  'marketing.home.how.cta': 'לשאלות נפוצות',

  'marketing.home.density.eyebrow': 'מערכת הצפיפות',
  'marketing.home.density.heading': 'שלבים שונים מצריכים החלטות שונות.',
  'marketing.home.density.note':
    'עוצמת הטיפול וההתאמה נקבעות לפי קריטריוני המלצה מאושרים, לעולם לא לפי ניקוד בלבד.',
  'marketing.home.density.mediaAlt': 'שלושה שלבי צפיפות קרקפת: גבוה, בינוני, מתקדם',
  'marketing.home.density.cta': 'עוד על דלילות השיער',

  'marketing.home.gray.eyebrow': 'המערכת לשיער אפור',
  'marketing.home.gray.heading': 'שיער אפור ראוי למערכת משלו.',
  'marketing.home.gray.body': 'שגרה מתואמת מבפנים ומבחוץ למי שמתמקד בניהול מראה השיער האפור.',
  'marketing.home.gray.mediaAlt': 'תוסף וסרום לשיער אפור יחד',
  'marketing.home.gray.forMen': 'לגברים',
  'marketing.home.gray.forWomen': 'לנשים',
  'marketing.home.gray.cta': 'עוד על שיער אפור',

  'marketing.home.science.eyebrow': 'מדע',
  'marketing.home.science.heading': 'לדעת מה יש בתוכנית שלך.',
  'marketing.home.science.note': 'מידע על הפורמולה והטענות כפוף לבדיקה רגולטורית.',
  'marketing.home.science.readMore': 'קריאה נוספת',
  'marketing.home.science.groupDensity': 'צפיפות',
  'marketing.home.science.groupGray': 'שיער אפור',
  'marketing.home.science.groupShampoo': 'שמפו',

  'marketing.home.progress.eyebrow': 'מעקב ROOTÉ',
  'marketing.home.progress.heading': 'התוכנית שלך לא נגמרת בקופה.',
  'marketing.home.progress.body':
    'עוקבים אחר המסע מהסריקה הראשונה ועד לסריקה הסופית. תמונות מודרכות עקביות מאפשרות השוואה לאורך זמן.',
  'marketing.home.progress.cta': 'איך המעקב עובד',
  'marketing.home.progress.baseline': 'בסיס',
  'marketing.home.progress.progressPhoto': 'תמונת מעקב',
  'marketing.home.progress.progressScan': 'סריקת מעקב',
  'marketing.home.progress.finalScan': 'סריקה סופית',
  'marketing.home.progress.before': 'לפני',
  'marketing.home.progress.after': 'אחרי',
  'marketing.home.progress.compareCaption':
    'כך עובד המעקב. התוצאות משתנות מיום ליום ותלויות במקרה האישי ובאופן השימוש במוצר.',
  'marketing.home.progress.beforeMediaAlt': 'מציין מיקום לסריקת בסיס',
  'marketing.home.progress.afterMediaAlt': 'מציין מיקום לסריקה סופית',

  'marketing.home.durations.eyebrow': 'משכי תוכנית',
  'marketing.home.durations.heading': 'טיפוח שיער הוא תוכנית, לא רכישה חד-פעמית.',
  'marketing.home.durations.note': 'מחירים, חיסכון והמוצרים הכלולים מאושרים בקופה.',
  'marketing.home.durations.includesDensity': 'טיפול לצפיפות השיער במידת ההתאמה',
  'marketing.home.durations.includesShampoo': 'שמפו לצמיחה מחדש',
  'marketing.home.durations.includesHeading': 'כל תוכנית כוללת',
  'marketing.home.durations.includesTracking': 'מעקב התקדמות בחשבון שלך',

  'marketing.home.faq.eyebrow': 'שאלות נפוצות',

  'marketing.home.finalCta.heading': 'להתחיל מהשיער שלך.',
  'marketing.home.finalCta.body': 'הצעד הראשון שלך הוא אבחון שיער מודרך וחינמי.',
  'marketing.home.finalCta.mediaAlt': 'סמל שלוש השערות של ROOTÉ עולה מתוך זקיק',

  // Legacy — still consumed by pre-redesign pages (About).
  'marketing.home.how.getStarted': 'להתחיל',
  'marketing.home.research.beforeLabel': 'לפני',
  'marketing.home.research.afterLabel': 'אחרי',
  'marketing.home.products.role.regrowth-stimulant': 'ממריץ צמיחה',
  'marketing.home.products.role.dht-blocker': 'חוסם DHT',
  'marketing.home.products.role.dht-support': 'תמיכה בחסימת DHT',
  'marketing.home.products.role.proprietary-support': 'תמיכה קניינית',

  // --- Solutions (WP4) ---
  'marketing.sol.indexHeading': 'להבין את השיער, ואז לבחור',
  'marketing.sol.indexBody': 'מסלולי שיער דליל ושיער אפור, כל אחד מתחיל באבחון חינם.',
  'marketing.sol.explore': 'לחקור את המסלול',
  'marketing.sol.exploreSystem': 'לחקור את השיטה',
  'marketing.sol.understandHeading': 'היכן זה מופיע',
  'marketing.sol.understandBody': 'שלושה אזורים שמבחינים בהם ראשונים — גלו איזה מהם תואם למה שאתם רואים.',
  'marketing.sol.systemEyebrow': 'התוכנית המומלצת',
  'marketing.sol.ctaHeading': 'להתחיל מהאבחון.',

  // --- Science (WP4) ---
  'marketing.sci.eyebrow': 'מדע',
  'marketing.sci.heading': 'לדעת מה יש בתוכנית שלך.',
  'marketing.sci.body': 'כל תוכנית בנויה מרכיבים בעלי מטרה מוגדרת. מידע על הפורמולה והטענות כפוף לבדיקה רגולטורית.',
  'marketing.sci.heroMediaAlt': 'הפורמולה של ROOTÉ, רכיבים פעילים בתאורת סטודיו',
  'marketing.sci.ingredientsHeading': 'רכיבים פעילים',
  'marketing.sci.groupDensity': 'צפיפות',
  'marketing.sci.groupGray': 'שיער אפור',
  'marketing.sci.groupShampoo': 'שמפו',
  'marketing.sci.readMore': 'קריאה נוספת',
  'marketing.sci.note': 'מידע על הפורמולה והטענות כפוף לבדיקה רגולטורית. איננו מציגים מחקר של רכיב או של מתחרה כשלנו.',
  'marketing.sci.mechanism.eyebrow': 'איך זה בנוי',
  'marketing.sci.mechanism.heading': 'איך הפורמולה שלך עובדת.',
  'marketing.sci.mechanism.dhtTitle': 'תמיכה במסלול ה-DHT',
  'marketing.sci.mechanism.dhtBody': 'רכיבים שנחקרו לתפקידם במסלול ה-DHT הקשור לנשירת שיער תורשתית.',
  'marketing.sci.mechanism.regrowthTitle': 'גירוי צמיחה מחדש',
  'marketing.sci.mechanism.regrowthBody': 'רכיבים מקומיים המשויכים לגירוי מחזור צמיחת השיער ולזרימת הדם בקרקפת.',
  'marketing.sci.mechanism.pigmentTitle': 'תמיכה בפיגמנט ובתזונה',
  'marketing.sci.mechanism.pigmentBody': 'רכיבים תזונתיים ופעילים המוזכרים בקטגוריית השיער האפור לתמיכה בתהליכי הפיגמנט הטבעיים.',
  'marketing.sci.mechanism.conditioningTitle': 'טיפוח קרקפת ושיער',
  'marketing.sci.mechanism.conditioningBody': 'צמחים ורכיבי הזנה התומכים בטיפוח היומיומי של הקרקפת והשיער.',
  'marketing.sci.status.approved': 'מאושר',
  'marketing.sci.status.working': 'התייחסות עבודה',
  'marketing.sci.status.requiresReview': 'ממתין לבדיקה',
  'marketing.sci.evidenceLabels.eyebrow': 'התקנים שלנו',
  'marketing.sci.evidenceLabels.heading': 'איך אנחנו מתייגים את הראיות.',
  'marketing.sci.evidenceLabels.body': 'לכל טענה על רכיב באתר הזה יש אחד משלושה תיוגים, כדי שתוכלו לראות כמה אנחנו בטוחים בה לפני שאתם קוראים אותה.',
  'marketing.sci.evidenceLabels.approvedBody': 'נבדק ואושר לשימוש בדיוק כפי שנכתב.',
  'marketing.sci.evidenceLabels.workingBody': 'התיאור שלנו: מדויק, אך אינו טענת יעילות ועדיין לא אושר רשמית.',
  'marketing.sci.evidenceLabels.requiresReviewBody': 'דורש ביסוס או בדיקה משפטית לפני שנציג טענה כלשהי, ומוצג כ-[ממתין] עד אז.',
  'marketing.sci.oversightEyebrow': 'פיקוח',
  'marketing.sci.oversightHeading': 'פיקוח רפואי',
  'marketing.sci.oversightBody': 'הפורמולה והטענות נבדקות מול ספרות רפואת עור לפני כל שילוח. תוכניות עם רכיב בעוצמת מרשם כוללות שלב של בדיקת טיפול.',
  'marketing.sci.oversightMediaAlt': 'רופא/ה בוחנים תוכנית טיפול',
  'marketing.sci.referencesNote': 'מקורות מתפרסמים לצד טענה לאחר אישורה.',
  'marketing.sci.ctaHeading': 'לראות מה האבחון שלך מציע.',

  // --- Our system (WP4) ---
  'marketing.sys.heading': 'לנתח. לטפל. לעקוב.',
  'marketing.sys.body': 'מסע אחד מחובר, מהסריקה הראשונה ועד לתוצאה הסופית, וכל יום שביניהם.',
  'marketing.sys.day': 'יום {n}',
  'marketing.sys.trackBody': 'התוכנית שלך כוללת נקודות סריקה וצילום בלוח זמנים קבוע. התאריכים מופיעים בחשבון עם תחילת התוכנית.',
  'marketing.sys.ctaHeading': 'להתחיל את המסע.',
  'marketing.sys.pillar.analyzeBody': 'הערכה קצרה וסריקת שיער מודרכת.',
  'marketing.sys.pillar.treatBody': 'תוכנית מומלצת עם שגרה יומית ברורה.',
  'marketing.sys.pillar.trackBody': 'השוואת התקדמות מהסריקה הראשונית ועד הסופית.',

  // --- Shop (WP4) ---
  'marketing.shop.heading': 'מערכת טיפוח שיער, שנבנתה כולה *סביבכם*.',
  'marketing.shop.body': 'בצעו אבחון שיער חינם וקבלו ערכת טיפול שנבנתה סביב הקרקפת, המטרות והתוצאות שלכם.',
  'marketing.shop.heroMediaAlt': 'מבחר המוצרים המלא',
  'marketing.shop.catalogHeading': 'כל המוצרים, במקום אחד.',
  'marketing.shop.filterLabel': 'סינון מוצרים',
  'marketing.shop.filterAll': 'הכול',
  'marketing.shop.filterThinning': 'דלילות',
  'marketing.shop.filterGray': 'אפור',
  'marketing.shop.bundles.eyebrow': 'צרור וחסכון',
  'marketing.shop.bundles.heading': 'להצטייד פעם אחת, לדלג על ההזמנה החוזרת.',
  'marketing.shop.bundles.body': 'המערכת המלאה, הטיפול בשיער אפור, וצמיחת שיער: כל חבילה באריזה לגברים ולנשים. חבילות המבוססות על טיפול הצפיפות מתווספות לתוכנית שלך לאחר בדיקת טיפול.',
  'marketing.shop.bundles.cta': 'הוספה לסל',
  'marketing.shop.bundles.save': 'חיסכון של {amount}',
  'marketing.shop.bundles.carousel.previous': 'תמונה קודמת',
  'marketing.shop.bundles.carousel.next': 'תמונה הבאה',
  'marketing.shop.bundles.carousel.goToImage': 'הצג תמונה {index}',
  'marketing.shop.finalCta.heading': 'לא בטוחים במה אתם צריכים?',
  'marketing.shop.finalCta.body': 'התחילו באבחון שיער חינמי וקבלו תוכנית שנבנתה סביב התוצאות שלכם.',

  // --- Product detail (WP4) ---
  'marketing.pdp.priceLabel': 'מחיר',
  'marketing.pdp.pricePending': 'בהמתנה',
  'marketing.pdp.fitTitle': 'איך זה משתלב בתוכנית ROOTÉ שלך',
  'marketing.pdp.fitBody': 'התוכנית שלך מפרטת בדיוק באילו מוצרים להשתמש ומתי. עמוד זה מסביר את המוצר עצמו.',
  'marketing.pdp.keyBenefits': 'יתרונות עיקריים',
  'marketing.pdp.overview': 'סקירה כללית',
  'marketing.pdp.format': '{size} ליחידה',
  'marketing.pdp.activesTitle': 'רכיבים פעילים עיקריים',
  'marketing.pdp.detailsTitle': 'פרטים',
  'marketing.pdp.howToUse': 'אופן השימוש',
  'marketing.pdp.safety': 'בטיחות והתאמה',
  'marketing.pdp.formula': 'התייחסות לפורמולה',
  'marketing.pdp.formulaNote': 'פירוט הפורמולה המלא זמין בדוח שלך לאחר בדיקת התאמה. כפוף לבדיקה רגולטורית.',
  'marketing.pdp.requiresReview': 'מצריך בדיקת טיפול',
  'marketing.pdp.noPrescriptionNeeded': 'ללא צורך במרשם',
  'marketing.pdp.alsoInFormula': 'גם בפורמולה הזו',
  'marketing.pdp.relatedTitle': 'מוצרים משלימים',
  'marketing.pdp.ctaHeading': 'להתחיל מהאבחון.',
  'marketing.pdp.storage': 'אחסון',
  'marketing.pdp.fullIngredients': 'רשימת מרכיבים מלאה',
  'marketing.pdp.supplementFacts.title': 'טבלת תוסף תזונה',
  'marketing.pdp.supplementFacts.servingSize': 'גודל מנה',
  'marketing.pdp.supplementFacts.servingsPerContainer': 'מנות באריזה',
  'marketing.pdp.supplementFacts.amountPerServing': 'כמות למנה',
  'marketing.pdp.supplementFacts.dailyValue': '% מהערך היומי',
  'marketing.pdp.supplementFacts.otherIngredients': 'מרכיבים נוספים',
  'marketing.pdp.supplementFacts.dvFootnote': '* הערך היומי לא נקבע.',
  'marketing.pdp.supplementFacts.allergenWarning': 'אזהרת אלרגנים',
  'marketing.pdp.badge.vegan': 'טבעוני',
  'marketing.pdp.badge.crueltyFree': 'ללא ניסויים בבעלי חיים',
  'marketing.pdp.badge.fragranceFree': 'ללא בישום',
  'marketing.pdp.badge.parabenFree': 'ללא פרבנים',
  'marketing.pdp.badge.sulfateFree': 'ללא סולפטים',
  'marketing.pdp.activesBody': 'הרכיבים הפעילים שעליהם מבוססת הפורמולה: מה כל אחד מהם עושה, ועד כמה חזקות הראיות שמאחוריו.',
  'marketing.pdp.directionsTitle': 'אופן שימוש ובטיחות',
  'marketing.pdp.directionsBody': 'איך למרוח, למי כדאי להימנע, ואיך לאחסן, הכול לפי התווית.',
  'marketing.pdp.detailsBody': 'פרטי הפורמולה והשאלות שאנחנו נשאלים עליה הכי הרבה.',
  'marketing.pdp.relatedBody': 'מהתוכנית שלך, נבחרו כדי לפעול יחד עם המוצר הזה.',
  'marketing.pdp.faqPrescription.q': 'האם נדרש מרשם או בדיקת טיפול?',
  'marketing.pdp.faqPrescription.aYes': 'כן, מוצר זה נוסף לתוכנית שלך רק לאחר שבדיקת טיפול מאשרת שהוא מתאים לך.',
  'marketing.pdp.faqPrescription.aNo': 'לא, מוצר זה אינו דורש מרשם או בדיקת טיפול.',
  'marketing.pdp.faqStrength.q': 'איך נבחרת הרמה שלי?',
  'marketing.pdp.faqStrength.a': 'אינך בוחר/ת את הרמה בעצמך. התוכנית שלך וכל שינוי בעוצמה נקבעים דרך ההערכה שלך ובדיקת טיפול, בהתאם למתאים לך.',

  'marketing.howItWorks.hero.title': 'איך ROOTÉ עובד',
  'marketing.howItWorks.hero.body': 'מהתמונה הראשונה שלך ועד לתוכנית המתמשכת, כך בדיוק נראה כל שלב.',
  'marketing.howItWorks.heroMediaAlt': 'התהליך, משאלון האבחון ועד לערכת הטיפול',
  'marketing.howItWorks.step1.title': 'שאלון הערכה',
  'marketing.howItWorks.step1.body': 'שאלון קצר על המטרות, ההיסטוריה ודפוס נשירת השיער שלך, כ-3 דקות.',
  'marketing.howItWorks.step1MediaAlt': 'טלפון מציג את שאלון ההערכה',
  'marketing.howItWorks.step2.title': 'ניתוח תמונת קרקפת מבוסס-AI',
  'marketing.howItWorks.step2.body': 'ארבע תמונות מודרכות ממופות מול סולם קליני מוכר לנשירת שיער.',
  'marketing.howItWorks.step3.title': 'הפורמולה שלך',
  'marketing.howItWorks.step3.body': 'הנתונים שלך קובעים אילו רכיבים פעילים ומשך זמן התוכנית שלך משתמשת בהם.',
  'marketing.howItWorks.step3MediaAlt': 'כרטיס התייחסות לפורמולה עם רכיבים פעילים',
  'marketing.howItWorks.step4.title': 'התאמות מתמשכות',
  'marketing.howItWorks.step4.body': 'בדיקות תקופתיות והשוואות תמונות מאפשרות לבחון מחדש את התוכנית שלך תוך כדי התקדמות.',
  'marketing.howItWorks.step4MediaAlt': 'השוואת תמונות מעקב לאורך זמן',
  'marketing.howItWorks.kit.title': 'מה כלול בערכה שלך',
  'marketing.howItWorks.philosophy.eyebrow': 'הגישה שלנו',
  'marketing.howItWorks.philosophy.heading': 'למה בנינו את זה ככה.',
  'marketing.howItWorks.timeline.title': 'למה לצפות',
  'marketing.howItWorks.timeline.m1': 'חודש 1',
  'marketing.howItWorks.timeline.m1Action': 'מתחילים כאן',
  'marketing.howItWorks.timeline.m1Bullet1': 'השגרה היומית שלכם מתחילה.',
  'marketing.howItWorks.timeline.m1Bullet2': 'תמונת המעקב הראשונה שלכם מתועדת.',
  'marketing.howItWorks.timeline.m3': 'חודש 3',
  'marketing.howItWorks.timeline.m3Action': 'שומרים על עקביות',
  'marketing.howItWorks.timeline.m3Bullet1': 'סריקת התקדמות משווה מול הסריקה הראשונית.',
  'marketing.howItWorks.timeline.m3Bullet2': 'התוכנית שלכם יכולה להתעדכן בבדיקה.',
  'marketing.howItWorks.timeline.m6': 'חודש 6',
  'marketing.howItWorks.timeline.m6Action': 'בודקים לאן הגעתם',
  'marketing.howItWorks.timeline.m6Bullet1': 'הסריקה הסופית שלכם מסיימת את תקופת האספקה הזו.',
  'marketing.howItWorks.timeline.m6Bullet2': 'אפשר לחדש, לעדכן, או לבצע אבחון חדש.',
  'marketing.howItWorks.timeline.shedding':
    'חלק מהמשתמשים מבחינים בעלייה זמנית בנשירה בשבועות הראשונים כשהזקיקים מאפסים את מחזור הצמיחה. זה צפוי ולרוב מתייצב.',
  'marketing.howItWorks.support.title': 'תמיכה לאורך הדרך',
  'marketing.howItWorks.support.body': 'שלחו הודעה לצוות הטיפול, תעדו תמונות, ובחנו מחדש את התוכנית שלכם ככל שהקרקפת משתנה.',
  'marketing.howItWorks.faq.title': 'שאלות נפוצות',
  'marketing.howItWorks.faq.q1': 'כמה זמן עד שאראה תוצאות?',
  'marketing.howItWorks.faq.a1': 'מחזורי צמיחת שיער הם איטיים. רוב התוכניות נבחנות על פני חודשים, לא שבועות.',
  'marketing.howItWorks.faq.q2': 'האם ניתוח התמונות פרטי?',
  'marketing.howItWorks.faq.a2': 'התמונות שלך משמשות רק לבניית התוכנית שלך ולמעקב אחר ההתקדמות שלך.',
  'marketing.howItWorks.cta.title': 'מוכנים לראות את התוכנית שלכם?',

  'marketing.aiSection.hero.title': 'איך עובדת סריקת השיער עם הבינה המלאכותית',
  'marketing.aiSection.hero.body':
    'שאלון קצר, ארבע תמונות מודרכות, וסריקה שממופה מול סולם נשירת שיער מוכר: כך זה עובד לפני שמקבלים תוכנית.',
  'marketing.aiSection.scan.eyebrow': 'הסריקה',
  'marketing.aiSection.scan.heading': 'שאלון קצר, ואז ארבע תמונות מודרכות.',
  'marketing.aiSection.cta.heading': 'מוכנים לראות את התוצאות שלכם?',
  'marketing.aiSection.cta.body': 'זה לוקח כמה דקות, וללא התחייבות.',
  'marketing.aiSection.timeline.progressPhotoAlt': 'תמונת סריקת התקדמות מודרכת, שצולמה באמצע התוכנית',
  'marketing.aiSection.measures.intro': 'כל סריקה בודקת את אותם שלושה דברים.',
  'marketing.aiSection.measures.densityBody': 'כמה כיסוי נראה לעין יש לך, אזור אחר אזור על פני הקרקפת.',
  'marketing.aiSection.measures.patternBody': 'היכן מופיעה הדלילות, בהתאם לסולם נורווד לגברים או לסולם לודוויג לנשים.',
  'marketing.aiSection.measures.progressionBody': 'בסיס להשוואה: כל תמונה מודרכת אחרי זו נמדדת מול נקודת ההתחלה שלך.',
  'marketing.aiSection.measures.progressionMediaAlt': 'מסך טלפון המשווה בין שתי תמונות קרקפת זו לצד זו',
  'marketing.aiSection.measures.densityMediaAlt': 'תמונת קרקפת מלמעלה המראה את צפיפות השיער והכיסוי',

  'marketing.science.hero.title': 'המדע שמאחורי התוכנית שלך',
  'marketing.science.hero.body': 'כל רכיב פעיל בפורמולה שלך מתועד למנגנון פעולה מפורסם.',
  'marketing.science.mechanism.title': 'איך נשירת שיער מדפוס עובדת',
  'marketing.science.mechanism.body':
    'DHT (דיהידרוטסטוסטרון) מכווץ בהדרגה זקיקים רגישים, מקצר את שלב הצמיחה במחזורים עוקבים. עצירת התהליך הזה, וגירוי ישיר של הזקיקים, הם שני המנופים שהפורמולה שלך בנויה סביבם.',
  'marketing.science.ingredients.title': 'רכיבים פעילים',
  'marketing.science.ingredients.evidence.regrowth-stimulant': 'נחקר רבות לגירוי צמיחה מחדש בנשירת שיער מדפוס.',
  'marketing.science.ingredients.evidence.dht-blocker': 'מפחית ייצור DHT בזקיק.',
  'marketing.science.ingredients.evidence.dht-support': 'תומך במצב הקרקפת לצד רכיבים ממוקדי DHT.',
  'marketing.science.ingredients.evidence.proprietary-support': 'התרכיב התומך הקנייני של ROOTÉ.',
  'marketing.science.evidence.title': 'ראיות קליניות',
  'marketing.science.evidence.body': 'אנחנו מדווחים נתוני מחקר אמיתיים ברגע שהם זמינים עבור פורמולה זו, לעולם לא נתון מומצא.',
  'marketing.science.evidence.beforeLabel': 'לפני',
  'marketing.science.evidence.afterLabel': 'אחרי',
  'marketing.science.evidence.caption': 'שערות מושרשות בקרקפת, בתקריב.',
  'marketing.science.advisory.title': 'פיקוח רפואי',
  'marketing.science.advisory.body': 'הפורמולה והטענות נבחנות מול הספרות הדרמטולוגית לפני שדבר יוצא לדרך.',
  'marketing.science.references.title': 'מקורות',
  'marketing.science.cta.title': 'ראו איך זה הופך לתוכנית שלכם',

  'marketing.products.hero.title': 'התוכנית שלך',
  'marketing.products.hero.body': 'פורמולה אחת, בנויה מרכיבים פעילים המותאמים לסריקה שלך, יחד עם השגרה התומכת בה.',
  'marketing.products.shampoo.title': 'ניקוי עדין לקרקפת',
  'marketing.products.shampoo.body': 'ניקוי יומי שמעוצב כך שלא ישטוף את הטיפול הטופיקלי.',
  'marketing.products.customized.title': 'מותאם לסריקה שלך',
  'marketing.products.customized.body':
    'רמת החומרה והאזורים המושפעים שלך, ממופים מהתמונות שלך מול סולם קליני מוכר לנשירת שיער, קובעים אילו רכיבים פעילים ומשך זמן התוכנית שלך משתמשת בהם. שום דבר אינו גנרי.',
  'marketing.products.guarantee.title': 'האחריות שלנו',
  'marketing.products.reviews.title': 'ראו מה אנשים אומרים',
  'marketing.products.catalog.title': 'כל מוצרי ROOTÉ',
  'marketing.products.range.viewAll': 'הצג הכול',
  'marketing.products.catalog.body':
    'כל מה שאנחנו מייצרים, מעבר לפורמולה האישית. לא בטוחים מאיפה להתחיל? התחילו באבחון חינם ונגיד לכם מה הקרקפת שלכם באמת צריכה.',
  'marketing.products.cat.growth': 'צמיחת שיער',
  'marketing.products.cat.wash': 'שמפו ומרכך',
  'marketing.products.cat.supplements': 'תוספי תזונה',
  'marketing.products.cat.lashBrow': 'ריסים וגבות',
  'marketing.products.cat.shower': 'מסנני מקלחת',
  'marketing.products.item.minoxidil5.desc': 'מינוקסידיל 5% מהמדף, למי שלא נמצא בתוכנית אישית.',
  'marketing.products.item.peptideOil.desc': 'שמן קליל שמסייע לבנות מחדש את הקשרים הפנימיים של השערה.',
  'marketing.products.item.bondShampoo.desc': 'תכשיר ניקוי מחזק שתומך במבנה הקשרים של השיער.',
  'marketing.products.item.scalpShampoo.desc': 'שמפו יומי ששומר על קרקפת רגועה ומאוזנת.',
  'marketing.products.item.conditioner.desc': 'מחזיר לחות והחלקה בלי להכביד על השיער.',
  'marketing.products.item.growthCaps.desc': 'כמוסות יומיות עם המיקרו-נוטריאנטים שצמיחת השיער נשענת עליהם.',
  'marketing.products.item.nutrientCaps.desc': 'משלים ברזל, אבץ וויטמיני B הקשורים לנשירה.',
  'marketing.products.item.marineCollagen.desc': 'קולגן ימי מתמוסס עם חומצה היאלורונית וויטמין C.',
  'marketing.products.item.browLash.desc': 'סרום מזין לגבות וריסים מלאים יותר למראה.',
  'marketing.products.item.browDensity.desc': 'סרום ממוקד לגבות דלילות או שנמרטו יתר על המידה.',
  'marketing.products.item.lashLength.desc': 'סרום לילי שמזין את קו הריסים.',
  'marketing.products.item.showerHead.desc': 'ראש מקלחת קבוע שמסנן מינרלים של מים קשים וכלור.',
  'marketing.products.item.showerHose.desc': 'ראש מקלחת נייד וצינור עם אותו סינון פנימי.',
  'marketing.products.item.mountedCartridge.desc': 'מחסנית סינון חלופית לראש המקלחת הקבוע.',
  'marketing.products.item.handheldCartridge.desc': 'מחסנית סינון חלופית לראש המקלחת הנייד.',

  'getStarted.title': 'הגדירו את חשבון המעקב שלכם',
  'getStarted.body': 'עדיין אין לכם חשבון מעקב. בצעו את אבחון השיער החינמי כדי לקבל תוכנית מותאמת אישית ולהתחיל לעקוב אחר ההתקדמות שלכם.',
  'getStarted.haveAccount': 'כבר יש לכם חשבון?',
  'getStarted.logIn': 'התחברות',

  'cart.open': 'פתיחת הסל',
  'cart.close': 'סגירה',
  'cart.title': 'הסל שלך',
  'cart.empty': 'הסל שלך ריק.',
  'cart.browse': 'עיון במוצרים',
  'cart.add': 'הוספה לסל',
  'cart.added': 'נוסף לסל',
  'cart.goToCart': 'מעבר לסל',
  'cart.decrease': 'הפחתת כמות',
  'cart.increase': 'הגדלת כמות',
  'cart.remove': 'הסרה',
  'cart.subtotal': 'סכום ביניים',
  'cart.checkout': 'למעבר לתשלום',
  'cart.viewBag': 'הצגת הסל המלא',

  'bag.title': 'הסל שלך',
  'bag.summary': 'סיכום ההזמנה',
  'bag.shipping': 'משלוח',
  'bag.total': 'סה"כ',
  'bag.checkout': 'מעבר לתשלום',
  'bag.continue': 'המשך קנייה',

  'bag.checkout.title': 'תשלום',
  'bag.checkout.summaryTitle': 'סיכום ההזמנה',
  'bag.checkout.edit': 'עריכת הסל',
  'bag.checkout.qty': 'כמות {qty}',
  'bag.checkout.back': 'חזרה לסל',
  'bag.checkout.error.payment': 'לא הצלחנו לעבד את התשלום. יש לנסות שוב.',

  'bag.success.title': 'ההזמנה התקבלה',
  'bag.success.body': 'תודה. ההזמנה שלך התקבלה ואישור בדרך לאימייל שלך.',
  'bag.success.orderId': 'מספר הזמנה',
  'bag.success.next': 'מה קורה עכשיו',
  'bag.success.next1': 'אישור יישלח לאימייל שלך בקרוב.',
  'bag.success.next2': 'אנחנו אורזים ושולחים תוך שני ימי עסקים.',
  'bag.success.next3': 'עקבו אחרי ההזמנה בכל עת מהחשבון שלכם.',
  'bag.success.trackOrder': 'לעקוב אחרי ההזמנה שלי',
  'bag.success.continue': 'המשך קנייה',
  'bag.success.home': 'חזרה לדף הבית',
  'bag.success.analysisTitle': "עוד לא עשית את אבחון השיער החינמי?",
  'bag.success.analysisBody': "אפשר לעשות אותו עכשיו — התוצאות וכיוון הטיפול יופיעו בחשבון שלך.",

  'marketing.about.hero.title': 'אודותינו',
  'marketing.about.mission.body':
    'ROOTÉ קיימת כדי להחליף ניחוש בתוכנית שנבנית מהקרקפת שלך: מבוססת ראיות, אישית, ושקופה לגבי מה שאנחנו יודעים ומה שעדיין לא.',
  'marketing.about.heroMediaAlt': 'סמל הזקיק של ROOTÉ',
  'marketing.about.problem.eyebrow': 'הפער שזיהינו',
  'marketing.about.problem.heading': 'הטיפול בנשירת שיער תקוע במצב אחד.',
  'marketing.about.problem.p1Title': 'שגרה אחת לכולם',
  'marketing.about.problem.p1Body': 'רוב המוצרים נמכרים באותו אופן לכולם, בלי קשר לדפוס, לחומרה או לסיבה.',
  'marketing.about.problem.p2Title': 'ביקור אצל רופא רק כדי להתחיל',
  'marketing.about.problem.p2Body': 'אפשרויות בעוצמת מרשם בדרך כלל דורשות קביעת תור עוד לפני שמתחילים.',
  'marketing.about.problem.p3Title': 'אין דרך לראות מה השתנה',
  'marketing.about.problem.p3Body': 'בלי תמונות או סריקות עקביות, קשה לדעת אם משהו באמת עובד.',
  'marketing.about.solution.eyebrow': 'איך אנחנו פותרים את זה',
  'marketing.about.solution.heading': 'סריקה אחת. תוכנית אחת שנבנית סביבה.',
  'marketing.about.solution.body': 'אתם משלימים אבחון קצר ומעלים ארבע תמונות מודרכות. אנו ממפים את מה שנראה לעין מול סולם נשירת שיער מוכר, ולאחר מכן מרכיבים תוכנית: המוצרים, השגרה, והאם בדיקת טיפול חלה עליכם.',
  'marketing.about.solution.mediaAlt': 'ארבע זוויות הסריקה המודרכת: קו השיער, הקודקוד, מלמעלה והצד',
  'marketing.about.benefits.eyebrow': 'מה תקבלו',
  'marketing.about.benefits.heading': 'נבנה סביב הסריקה שלכם, לא מדף עם אותו בקבוק לכולם.',
  'marketing.about.benefits.b1Title': 'מותאם לסריקה שלכם',
  'marketing.about.benefits.b1Body': 'ההמלצה שלכם מגיעה ממה שהתמונות שלכם באמת מראות, לא משגרה גנרית.',
  'marketing.about.benefits.b2Title': 'בדיקה, לא ניחוש',
  'marketing.about.benefits.b2Body': 'כל רכיב בעוצמת מרשם מאושר בבדיקת טיפול לפני המשלוח.',
  'marketing.about.benefits.b3Title': 'דרך לראות שינוי',
  'marketing.about.benefits.b3Body': 'תמונות וסריקות מעקב עקביות מאפשרות להשוות בין הסריקה הראשונית למצב הנוכחי.',
  'marketing.about.comparison.eyebrow': 'ההבדל',
  'marketing.about.comparison.heading': 'הגישה הרגילה, והגישה שלנו.',
  'marketing.about.comparison.colGeneric': 'גישה רגילה',
  'marketing.about.comparison.colOurs': 'הגישה שלנו',
  'marketing.about.comparison.row1Label': 'התאמה אישית',
  'marketing.about.comparison.row1Generic': 'אותו מוצר לכולם',
  'marketing.about.comparison.row1Ours': 'מותאם לסריקה שלכם',
  'marketing.about.comparison.row2Label': 'גישה לעוצמת מרשם',
  'marketing.about.comparison.row2Generic': 'ביקור נפרד אצל רופא',
  'marketing.about.comparison.row2Ours': 'משולב באבחון שלכם, עם בדיקת טיפול',
  'marketing.about.comparison.row3Label': 'מעקב התקדמות',
  'marketing.about.comparison.row3Generic': 'אין דרך מובנית לבדוק',
  'marketing.about.comparison.row3Ours': 'תמונות וסריקות מעקב בלוח זמנים קבוע',
  'marketing.about.comparison.row4Label': 'טענות על רכיבים',
  'marketing.about.comparison.row4Generic': 'לרוב לא מתויגות',
  'marketing.about.comparison.row4Ours': 'כל טענה מתויגת כמאושרת, כהתייחסות עבודה, או כממתינה לבדיקה',
  'marketing.about.story.title': 'למה התחלנו',
  'marketing.about.story.body':
    'נשירת שיער מדפוס נפוצה, נחקרת היטב, ועדיין בדרך כלל מטופלת בשגרה אחידה לכולם. התחלנו את זה כדי שכל תוכנית תתחיל מסריקה אמיתית במקום.',
  'marketing.about.story.mediaAlt': 'תקריב של סריקת קרקפת, נקודת ההתחלה של כל תוכנית',
  'marketing.about.promise.eyebrow': 'ההתחייבות שלנו',
  'marketing.about.promise.heading': 'למה אנחנו מתחייבים, בכתב.',
  'marketing.about.promise.p1Title': 'כלום לא מומצא',
  'marketing.about.promise.p1Body': 'אם אין לנו עדיין מידע אמיתי, הוא מסומן כממתין, לעולם לא מנוחש.',
  'marketing.about.promise.p2Title': 'בדיקה לפני כל דבר בעוצמת מרשם',
  'marketing.about.promise.p2Body': 'כל רכיב בעוצמת מרשם מאושר בבדיקת טיפול לפני המשלוח.',
  'marketing.about.promise.p3Title': 'כל טענה מתויגת',
  'marketing.about.promise.p3Body': 'טענות על רכיבים מתויגות כמאושרות, כהתייחסות עבודה, או כממתינות לבדיקה, כדי שתדעו בדיוק כמה אנחנו בטוחים.',
  'marketing.about.team.eyebrow': 'מי עומד מאחורי זה',
  'marketing.about.team.title': 'צוות ויועצים',
  'marketing.about.team.body': 'אנחנו משלימים את הביוגרפיות וההסמכות של האנשים והיועצים בצוות הזה.',
  'marketing.about.values.title': 'איך אנחנו עובדים',
  'marketing.about.values.v1.title': 'מבוסס ראיות',
  'marketing.about.values.v1.body': 'כל רכיב פעיל מתועד למנגנון מפורסם.',
  'marketing.about.values.v2.title': 'אישי',
  'marketing.about.values.v2.body': 'התוכנית שלך מתחילה מהסריקה שלך, לא משגרה גנרית.',
  'marketing.about.values.v3.title': 'שקוף',
  'marketing.about.values.v3.body': 'מה שאנחנו עדיין לא יודעים, אנחנו מסמנים כ״ממתין״, לעולם לא ממציאים.',
  'marketing.about.press.title': 'תקשורת',
  'marketing.about.careers.title': 'קריירה',
  'marketing.about.careers.body': 'אנחנו בונים את הצוות מאחורי זה.',
  'marketing.about.cta.title': 'קבלו את התוכנית שלכם',

  'marketing.faq.hero.title': 'שאלות נפוצות',
  'marketing.faq.hero.body': 'תשובות למה שאנשים שואלים לפני, במהלך ואחרי תחילת תוכנית.',
  'marketing.faq.category.gettingStarted': 'התחלה',
  'marketing.faq.category.plan': 'התוכנית',
  'marketing.faq.category.ingredients': 'רכיבים ובטיחות',
  'marketing.faq.category.billing': 'חיוב ומשלוח',
  'marketing.faq.gettingStarted.q1': 'מה אני צריך כדי להתחיל?',
  'marketing.faq.gettingStarted.a1': 'רק כמה דקות ומצלמת טלפון. השאלון והסריקה לוקחים כ-3 דקות.',
  'marketing.faq.gettingStarted.q2': 'האם הסריקה מדויקת?',
  'marketing.faq.gettingStarted.a2': 'היא ממפה את התמונות שלך מול סולם קליני מוכר לנשירת שיער.',
  'marketing.faq.plan.q1': 'האם אפשר לשנות את התוכנית שלי בהמשך?',
  'marketing.faq.plan.a1': 'כן, ניתן לבחון מחדש את התוכנית שלך ככל שהקרקפת והצרכים שלך משתנים.',
  'marketing.faq.ingredients.q1': 'האם הרכיבים בטוחים?',
  'marketing.faq.ingredients.a1': 'כל רכיב פעיל הוא תרכובת נחקרת היטב לנשירת שיער מדפוס. ראו את עמוד המדע לפרטים.',
  'marketing.faq.support.title': 'עדיין יש לכם שאלות?',
  'marketing.faq.cta.body': 'התחילו באבחון שיער חינם וקבלו תוכנית שנבנתה סביב התוצאות שלכם.',
  'marketing.faq.support.cta': 'צרו קשר עם התמיכה',

  'marketing.support.hero.title': 'תמיכה',
  'marketing.support.hero.body': 'אנחנו כאן לעזור לפני, במהלך ואחרי התוכנית שלכם.',
  'marketing.support.contact.title': 'צרו קשר',
  'marketing.support.contact.inApp': 'שלחו הודעה לצוות הטיפול ישירות מהחשבון שלכם ברגע שיש לכם תוכנית.',
  'marketing.support.form.title': 'שלחו לנו הודעה',
  'marketing.support.form.nameLabel': 'שם',
  'marketing.support.form.emailLabel': 'אימייל',
  'marketing.support.form.topicLabel': 'נושא',
  'marketing.support.form.topic1': 'התחלה',
  'marketing.support.form.topic2': 'התוכנית שלי',
  'marketing.support.form.topic3': 'חיוב ומשלוח',
  'marketing.support.form.topic4': 'משהו אחר',
  'marketing.support.form.messageLabel': 'הודעה',
  'marketing.support.form.submit': 'שליחה',
  'marketing.support.form.stubNotice': 'תודה. הטופס הזה אינו מחובר בגרסת התצוגה. לא נשלח דבר.',
  'marketing.support.help.title': 'עיינו בשאלות הנפוצות',
  'marketing.support.help.cta': 'לשאלות נפוצות',
  'marketing.support.cta.title': 'מוכנים להתחיל?',

  'marketing.legal.terms.title': 'תנאים והגבלות',
  'marketing.legal.privacy.title': 'מדיניות פרטיות',
  'marketing.legal.updated': 'עודכן לאחרונה',

  // כוונת הפתיחה של מדיניות הפרטיות. גוף הסעיפים העיקרי של /terms ו-/privacy עבר לשכבת
  // התוכן (src/content/legal.ts LEGAL_BODIES, מתוך חבילת המסמכים המשפטיים של ROOTÉ, 2026-09-22)
  // כדי שגורמי המשפט/מוצר יוכלו להחליף את הטקסט בלי לגעת במפתחות ה-i18n.
  'marketing.legal.privacy.intro':
    'מדיניות זו מסבירה אילו מידע ROOTÉ אוספת, למה, ואיך אפשר לשלוט בכך.',

  // פרטי החברה / הישות המשפטית — מוצגים ב-/terms (העובדות ב-roote.config company)
  'marketing.legal.company.title': 'פרטי החברה',
  'marketing.legal.company.intro': 'ROOTÉ הוא מותג המופעל על ידי החברה שלהלן.',
  'marketing.legal.company.legalNameLabel': 'שם החברה הרשום',
  'marketing.legal.company.entityTypeLabel': 'סוג הישות',
  'marketing.legal.company.entityType': 'חברה בערבון מוגבל (LLC), ממוסה כ-S Corporation',
  'marketing.legal.company.countryLabel': 'מדינת הרישום',
  'marketing.legal.company.country': 'ארצות הברית, קליפורניה',
  'marketing.legal.company.regNumberLabel': 'מספר רישום',
  'marketing.legal.company.einLabel': 'מספר מס פדרלי (EIN)',
  'marketing.legal.company.incorporatedLabel': 'תאריך ההתאגדות',
  'marketing.legal.company.representativeLabel': 'מורשה חתימה',
  'marketing.legal.company.addressLabel': 'כתובת רשומה',
  'marketing.legal.company.emailLabel': 'אימייל',
  'marketing.legal.company.phoneLabel': 'טלפון',

  // עמוד התמיכה — פרטי יצירת קשר (מחליף את מצייני ה-pending)
  'marketing.support.contact.emailLabel': 'אימייל',
  'marketing.support.contact.phoneLabel': 'טלפון',
  'marketing.support.contact.hoursLabel': 'שעות',
  'marketing.support.contact.hours': 'ראשון–חמישי, 9:00–17:00',

  // כותרת תחתונה — שורת הישות + קישור לתנאי מכר
  'marketing.footer.brandOf': 'ROOTÉ הוא מותג של',
  'marketing.footer.termsOfSale': 'תנאי מכר',

  'analysis.finalizing': 'מסיימים את הניתוח…',

  // --- Free Hair Analysis flow (WP5) ---
  'analysis.nav.startOverTitle': 'להתחיל מחדש?',
  'analysis.nav.startOverBody':
    'פעולה זו תמחק את התשובות והתמונות שלך באבחון הנוכחי. החשבון שלך ותוכנית שכבר רכשת (אם יש) לא ייפגעו.',
  'analysis.nav.startOverConfirm': 'כן, להתחיל מחדש',

  'analysis.intro.title': 'אבחון השיער שלך מתחיל כאן.',
  'analysis.intro.point1': 'כמה דקות',
  'analysis.intro.point2': 'תמונות מודרכות',
  'analysis.intro.point3': 'תוצאה אישית',
  'analysis.intro.cta': 'להתחלת האבחון',
  'analysis.intro.consentHint': 'את/ה בוחר/ת מה לשתף. שום דבר לא נשלח בלי הסכמתך.',

  'analysis.gender.title': 'איך נתאים עבורך את חוויית ROOTÉ?',
  'analysis.gender.note': 'זה קובע כיצד מוצגת האריזה. המלצות הטיפול נובעות מהאבחון ומכללי ההתאמה, לא מהבחירה הזו.',
  'analysis.gender.packagingTitle': 'בחרו את אריזת ROOTÉ המועדפת עליכם',

  'analysis.age.title': 'מה טווח הגיל שלך?',

  'analysis.previousProducts.title': 'האם ניסית בעבר מוצרים לנשירת שיער?',
  'analysis.satisfaction.title': 'האם היית מרוצה מהמוצרים שניסית?',

  'analysis.goal.title': 'מה המטרה הראשית שלך?',

  'analysis.pattern.title': 'איזו תמונה הכי דומה לדפוס השיער הנוכחי שלך?',
  'analysis.pattern.body': 'בחר/י את ההתאמה הקרובה ביותר. תוכל/י לחזור אחורה ולשנות את הבחירה שלך.',

  'analysis.photos.title': 'תמונות הבסיס שלך',
  'analysis.photos.body': 'ארבע תמונות מהטלפון שלך: חזית, מלמעלה, קודקוד וקו השיער. אלו הופכות לרשומת הבסיס שלך למעקב אחר התקדמות לאורך זמן — תשובות השאלון שלך, לא התמונות הללו, הן שמניעות את ההמלצה של היום.',
  'analysis.photos.needConsent': 'יש לאשר את הסכמת התמונות למעלה כדי להמשיך.',
  'analysis.photos.needAll': 'נוספו {have} מתוך {total} תמונות. יש להוסיף את כל ארבע הזוויות כדי להמשיך.',

  'analysis.consent.label': 'אני מסכים/ה לשתף את התמונות האלה לצורך אבחון השיער שלי.',
  'analysis.consent.summary': 'משמשות רק לבניית פרופיל השיער והדוח שלך.',
  'analysis.consent.details':
    'התמונות נדרשות כדי ש-ROOTÉ תוכל לארגן את מה שנראה בשיער. בגרסת התצוגה הזו הן נשארות במכשיר שלך. בגרסה המלאה הן היו מעובדות ליצירת האבחון, נשמרות לתקופה המצוינת במדיניות הפרטיות ונמחקות לפי בקשה. ROOTÉ אינה טוענת לטיפול בסטנדרט HIPAA או רפואי.',

  'analysis.scanning.title': 'מנתחים את השיער שלך…',
  'analysis.scanning.body': 'הסריקה שלך מאורגנת בזמן שאת/ה עונה על כמה שאלות.',
  'analysis.scanning.percent': 'מנתח/ת… {pct}%',

  'analysis.results.title': 'אבחון השיער שלך מוכן',
  'analysis.results.cardTitle': 'פרופיל השיער שלך',
  'analysis.results.rowDensity': 'צפיפות שיער',
  'analysis.results.rowPattern': 'העניין',
  'analysis.results.rowProgression': 'סיכום',
  'analysis.results.rowStage': 'דפוס אפור',
  'analysis.results.rowArea': 'בולט ביותר',
  'analysis.results.rowPace': 'התקדמות',
  'analysis.results.grayStage.early': 'מוקדם',
  'analysis.results.grayStage.moderate': 'בינוני',
  'analysis.results.grayStage.advanced': 'מתקדם',
  'analysis.results.emailLabel': 'לאן לשלוח את התוצאות שלך?',
  'analysis.results.emailCta': 'שליחת התוצאות האישיות שלי',
  'analysis.results.consentLine': 'התוצאות שלך נפתחות במסך הבא. הודעות על התוכנית והחשבון הן תפעוליות; דיוור שיווקי נפרד ואופציונלי.',
  'analysis.results.consentResults': 'שלחו לי במייל את תוצאות ROOTÉ ועדכוני התוכנית',
  'analysis.results.consentResultsRequired': 'אנחנו צריכים את זה כדי לשלוח לך את התוצאות.',
  'analysis.results.consentMarketing': 'אשמח לקבל גם חדשות והצעות מ-ROOTÉ (רשות)',

  'gray.area.temples': 'רקות',
  'gray.area.crown': 'קודקוד וחלק עליון',
  'gray.area.throughout': 'בכל הראש',
  'gray.pace.slow': 'איטי',
  'gray.pace.steady': 'קבוע',
  'gray.pace.fast': 'מהיר',
  'gray.summary.early': 'מספר שערות אפורות, בדרך כלל ברקות.',
  'gray.summary.moderate': 'האפור בולט בקודקוד ובחלק העליון.',
  'gray.summary.advanced': 'האפור מפוזר בכל השיער.',

  'app.nav.label': 'תוכנית',
  'app.nav.today': 'היום',
  'app.nav.plan': 'התוכנית שלי',
  'app.nav.progress': 'התקדמות',
  'app.nav.photos': 'תמונות',
  'app.nav.scans': 'סריקות',
  'app.nav.orders': 'הזמנות',
  'app.nav.subscription': 'מנוי',
  'app.nav.care': 'צוות הליווי',
  'app.nav.profile': 'פרופיל',
  'app.nav.shop': 'חנות מוצרים',

  'app.orders.title': 'הזמנות',
  'app.orders.empty': 'אין הזמנות עדיין.',
  'app.orders.kindProgram': 'תוכנית',
  'app.orders.kindBag': 'מוצרים',
  'app.orders.note': 'היסטוריית ההזמנות נשמרת במכשיר הזה בגרסת התצוגה.',

  'app.subscription.title': 'מנוי',
  'app.subscription.status': 'הזמנה חוזרת אוטומטית',
  'app.subscription.on': 'פעיל',
  'app.subscription.off': 'כבוי',
  'app.subscription.cadence': 'נשלח כל',
  'app.subscription.nextCharge': 'החיוב הבא',
  'app.subscription.enable': 'הפעלת הזמנה חוזרת אוטומטית',
  'app.subscription.cancel': 'ביטול הזמנה חוזרת אוטומטית',
  'app.subscription.cancelConfirmBody': 'התוכנית הנוכחית שלך אינה מושפעת. ההזמנה החוזרת נעצרת ולא תחויב/י שוב אלא אם תפעיל/י מחדש.',
  'app.subscription.cancelConfirm': 'אישור ביטול',
  'app.subscription.legal': 'חיוב מתחדש, תדירות משלוח, מחיר החידוש, מסים ומשלוח מוצגים לפני ההפעלה. ניתן לבטל כאן בכל עת, וזכויות הגנת הצרכן המקומיות שלך נשמרות.',
  'app.noProgram.body': 'התחילו אבחון שיער חינם כדי לקבל תוכנית.',
  'app.noProgram.devSeedCta': 'טעינת תוכנית הדגמה (למפתחים בלבד)',
  'app.preDelivery.title': 'התוכנית שלך בדרך',
  'app.preDelivery.subtitle': 'הנה למה לצפות לפני היום הראשון שלך.',
  'app.preDelivery.planReady': 'התוכנית שלך מוכנה',
  'app.preDelivery.orderTitle': 'ההזמנה שלך',
  'app.preDelivery.orderedOn': 'הוזמן בתאריך {date}',
  'app.preDelivery.shippingNote': 'ההזמנה שלך בהכנה ובדרך אליך. בגרסת התצוגה הזו אין מעקב משלוחים חי, כך שברגע שהחבילה תגיע, יש לאשר זאת למטה כדי להתחיל את התוכנית.',
  'app.preDelivery.supportTitle': 'יש לך שאלות לגבי ההזמנה?',
  'app.preDelivery.supportBody': 'אפשר לפנות אלינו בכל עת.',
  'app.preDelivery.confirmCta': 'החבילה הגיעה? אשרו כדי להתחיל את היום הראשון שלכם',
  'app.preDelivery.viewShipping': 'צפייה בפרטי המשלוח',
  'app.preDelivery.orderedStage': 'ההזמנה בוצעה',
  'app.preDelivery.preparingStage': 'בהכנה ובמשלוח',
  'app.preDelivery.whileYouWaitTitle': 'בזמן שאת/ה ממתין/ה',
  'app.preDelivery.reviewPlanRow': 'סקירת התוכנית שלך',
  'app.preDelivery.reviewPlanTitle': 'איך להשתמש בתוכנית שלך',
  'app.preDelivery.contactRow': 'יצירת קשר עם התמיכה',
  'app.preDelivery.shipping.title': 'פרטי משלוח',
  'app.preDelivery.shipping.orderSummary': 'סיכום ההזמנה',
  'app.preDelivery.shipping.priceLabel': 'סך ההזמנה',
  'app.preDelivery.shipping.supportCenter': 'מרכז תמיכה',
  'app.preDelivery.shipping.refundRow': 'בקשת החזר או החזרה',
  'app.preDelivery.shipping.contactRow': 'יצירת קשר עם התמיכה',
  'app.preDelivery.shipping.helpRow': 'מרכז עזרה',
  'app.preDelivery.refund.title': 'בקשת החזר או החזרה',
  'app.preDelivery.refund.intro': 'ספרו לנו מה קרה ונבדוק את הבקשה שלכם.',
  'app.preDelivery.refund.reasonLabel': 'סיבה',
  'app.preDelivery.refund.reasonDamaged': 'מוצר פגום',
  'app.preDelivery.refund.reasonNotExpected': 'לא מה שציפיתי לו',
  'app.preDelivery.refund.reasonNoLongerNeeded': 'כבר לא נחוץ לי',
  'app.preDelivery.refund.reasonOther': 'אחר',
  'app.preDelivery.refund.detailsLabel': 'פרטים (לא חובה)',
  'app.preDelivery.refund.detailsPlaceholder': 'ספרו לנו עוד...',
  'app.preDelivery.refund.submit': 'שליחת בקשה',
  'app.preDelivery.refundSent.title': 'הבקשה נשלחה',
  'app.preDelivery.refundSent.body': 'קיבלנו את בקשתכם ונחזור אליכם במייל.',
  'app.preDelivery.refundSent.back': 'חזרה לפרטי המשלוח',
  'app.preDelivery.contact.title': 'יצירת קשר עם התמיכה',
  'app.preDelivery.contact.body': 'הצוות שלנו זמין במייל או בטלפון לשאלות לגבי ההזמנה, התוכנית או המוצרים שלכם.',
  'app.preDelivery.help.title': 'מרכז עזרה',
  'app.preDelivery.help.q1': 'מתי ההזמנה שלי תגיע?',
  'app.preDelivery.help.a1': 'בגרסת התצוגה הזו אין מעקב משלוחים חי. ברגע שהחבילה תגיע, חיזרו לכאן ואשרו זאת כדי להתחיל את התוכנית.',
  'app.preDelivery.help.q2': 'איך התוכנית שלי עובדת?',
  'app.preDelivery.help.a2': 'התוכנית משלבת את המוצרים והשגרה שהערכה שלכם התאימה לכם, ומתועדת החל מהיום שבו תאשרו את קבלת המשלוח.',
  'app.preDelivery.help.q3': 'אפשר לשנות את התוכנית או המוצרים שלי?',
  'app.preDelivery.help.a3': 'צרו קשר עם התמיכה ונעזור לכם לעדכן את התוכנית או המוצרים.',
  'app.preDelivery.help.q4': 'מה עושים אם מוצר מגיע פגום?',
  'app.preDelivery.help.a4': 'יש להגיש בקשת החזר או החזרה עם פרטים על הנזק.',
  'app.task.pendingName': 'הטיפול שלך',

  'app.today.title': 'היום',
  'app.today.dayLabel': 'יום {day} מתוך {total}',
  'app.today.remaining': 'נותרו {days} ימים · מסתיים ב-{end}',
  'app.today.reorder.title': 'הגיע הזמן להזמין מחדש',
  'app.today.reorder.body': 'המלאי הנוכחי שלך עומד להיגמר. הזמן עכשיו כדי שהתוכנית תימשך ללא הפסקה.',
  'app.today.reorder.cta': 'הזמנת השלב הבא',
  'app.today.checklist.title': 'השגרה של היום',
  'app.today.adherence.label': 'היצמדות',
  'app.today.adherence.note': 'משימות שהושלמו ב-7 הימים האחרונים.',
  'app.today.nextOrder.label': 'ההזמנה הבאה',
  'app.today.nextOrder.note': 'נזכיר לך לפני שהמלאי יאזל.',
  'app.today.reminders.title': 'תזכורות',
  'app.today.reminders.body': 'תזכורות לכל שלב נשלחות כאן באפליקציה בשעות הטיפול שלך.',
  'app.today.link.progress': 'הוספת תמונות השבוע',
  'app.today.link.care': 'שליחת הודעה לצוות הליווי',

  // --- לוח בקרה: סקירה + היום (Track 2 WP-B) ---
  'app.nav.overview': 'סקירה',
  'app.nav.support': 'תמיכה',
  'app.nav.beforeAfter': 'לפני ואחרי',

  'app.overview.eyebrow': 'ROOTÉ שלי',
  'app.overview.dayOf': 'יום {day} מתוך {total}',
  'app.overview.completedRemaining': '{done} ימים הושלמו · נותרו {left} ימים',
  'app.overview.continueToday': 'להמשך התוכנית של היום',
  'app.overview.viewProgress': 'לצפייה בהתקדמות שלי',
  'app.overview.nextCheckpoint': 'נקודת הבדיקה הבאה',
  'app.overview.allCheckpointsDone': 'כל נקודות הבדיקה הושלמו',
  'app.overview.reorderDue': 'הגיע הזמן להזמין מחדש.',
  'app.overview.programComplete': 'התוכנית שלך הושלמה',
  'app.overview.programCompleteBody': 'דוח התוצאות המלא שלך מוכן: לפני ואחרי, ניתוח ראשוני וסופי, וההתמדה שלך.',
  'app.overview.viewResults': 'לצפייה בתוצאות שלך',

  'app.status.onTrack': 'במסלול',
  'app.status.keepGoing': 'ממשיכים',
  'app.status.catchUp': 'להשלים פערים',
  'app.status.complete': 'הושלם',

  'app.checkpoint.baseline': 'סריקת שיער ראשונית',
  'app.checkpoint.photo': 'תמונות מעקב',
  'app.checkpoint.scan': 'סריקת מעקב',
  'app.checkpoint.finalScan': 'סריקת שיער סופית',
  'app.checkpoint.state.completed': 'הושלם',
  'app.checkpoint.state.skipped': 'דילגת',
  'app.checkpoint.state.due': 'עכשיו',
  'app.checkpoint.state.overdue': 'עבר המועד',
  'app.checkpoint.state.upcoming': 'בקרוב',
  'app.checkpoint.skip': 'דילוג',
  'app.checkpoint.unskip': 'ביטול דילוג',
  'app.checkpoint.completeNow': 'להשלים עכשיו',

  'app.today.slot.morning': 'בוקר',
  'app.today.slot.evening': 'ערב',
  'app.today.slot.shampoo': 'שמפו',
  'app.today.dose.applyScalp': 'למרוח על האזורים הרלוונטיים',
  'app.today.dose.weekly': 'שבועי',
  'app.today.dose.shampoo': 'להשתמש במקום השמפו הרגיל',
  'app.today.dose.capsules2': '2 קפסולות',
  'app.today.dose.serum': 'מספר טיפות, לעסות פנימה',
  'app.today.markDone': 'סימון כבוצע',
  'app.today.markPending': 'סימון כלא בוצע',
  'app.today.skip': 'לדלג היום',
  'app.today.unskip': 'ביטול דילוג',
  'app.today.greeting': 'ברוכים השבים',
  'app.today.goalLabel': 'המטרה שלך',
  'app.today.nextTask.label': 'הבא בתור',
  'app.today.empty.body': 'השגרה היומית שלך תופיע כאן ברגע שהתוכנית שלך תכלול טיפול אחד לפחות.',
  'app.today.empty.cta': 'צפו בתוכנית שלכם',

  // --- בסיס / תמונות / סריקות (Track 2 WP-C) ---
  'app.metric.key.visibleThinning': 'דלילות נראית',
  'app.metric.key.hairline': 'קו השיער',
  'app.metric.key.lossArea': 'אזור הנשירה',
  'app.metric.key.scalpCondition': 'מצב הקרקפת',
  'app.metric.level.low': 'נמוך',
  'app.metric.level.medium': 'בינוני',
  'app.metric.level.high': 'גבוה',
  'app.metric.level.unknown': 'לא הוערך',

  'app.baseline.title': 'הבסיס שלך',
  'app.baseline.badge': 'לפני',
  'app.baseline.date': 'תאריך הבסיס',
  'app.baseline.programStart': 'תחילת התוכנית',
  'app.baseline.photos': 'תמונות ראשוניות',
  'app.baseline.analysisTitle': 'ניתוח ראשוני',
  'app.baseline.metricsNote': 'אלה קריאות איכותיות מהאבחון הראשון שלך. מדדים מספריים יופיעו לאחר חיבור שירות ניתוח.',

  'app.photos.title': 'תמונות מעקב',
  'app.photos.body': 'צלמו את אותן ארבע הזוויות בכל נקודת בדיקה. קו המתאר והצללית של התמונה הקודמת עוזרים ליישר כל צילום מול הבסיס.',
  'app.photos.notYet': 'אין תמונה עדיין',
  'app.photos.markCheckpointDone': 'שמירת נקודת הבדיקה',

  'app.scans.title': 'סריקות שיער',
  'app.scans.body': 'חלק מנקודות הבדיקה חוזרות על הסריקה המקורית כדי להשוות את אותן הזוויות לאורך זמן.',
  'app.scans.hairhealthCta.title': 'מעדיפים סריקה חוזרת מודרכת?',
  'app.scans.hairhealthCta.body': 'השותפה שלנו HairHealth.ai יכולה גם היא ללוות אתכם בסריקה חוזרת ולהעביר את התוצאה לצוות שלנו.',
  'app.scans.hairhealthCta.cta': 'סריקה חוזרת עם HairHealth.ai',
  'app.scans.progressPrompt': 'הגיע הזמן לסריקת המעקב שלך',
  'app.scans.progressBody': 'נשווה את השיער שלך באמצעות אותן ארבע זוויות מהניתוח הראשון.',
  'app.scans.finalPrompt': 'סריקת השיער הסופית של ROOTÉ',
  'app.scans.finalBody': 'סיימת את התוכנית. חזור על הסריקה המקורית כדי להשוות את נקודת ההתחלה עם התוצאה הסופית.',
  'app.scans.startCapture': 'התחלת סריקה',
  'app.scans.runAnalysis': 'ניתוח ההתקדמות שלי',
  'app.scans.analyzing': 'מנתחים את ההתקדמות שלך…',
  'app.scans.history': 'היסטוריית סריקות',
  'app.scans.none': 'הסריקות שתצלמו יופיעו כאן — התחילו סריקה למעלה כשהיא מגיעה.',
  'app.scans.demo': 'נתוני הדגמה',
  'app.scans.type.baseline': 'סריקת בסיס',
  'app.scans.type.progress': 'סריקת מעקב',
  'app.scans.type.final': 'סריקה סופית',

  'app.progress.title': 'התקדמות לאורך זמן',
  'app.progress.subtitle': 'איך כל חלק מהאבחון שלך השתנה מאז נקודת הפתיחה.',
  'app.progress.add.title': 'הוספת תמונות התקדמות',
  'app.progress.compare.title': 'לפני ואחרי',
  'app.progress.compare.empty': 'אין תמונה עדיין',
  'app.progress.timeline.title': 'ציר הזמן שלך',
  'app.progress.metricsTitle': 'מאז נקודת הפתיחה',
  'app.progress.baselineLabel': 'נקודת פתיחה',
  'app.progress.latestLabel': 'עדכני',
  'app.progress.compareWith': 'להשוות מול',
  'app.progress.noScanYet': 'סריקת נקודת הבדיקה הראשונה שלך תמלא זאת.',
  'app.progress.noBaseline': 'עדיין אין לנו נתוני בסיס להשוואה. אלה מגיעים בדרך כלל מהניתוח הראשוני של השיער שלכם.',
  'app.progress.metricsNote': 'אלה קריאות איכותניות. מדדים מספריים יופיעו לאחר חיבור שירות ניתוח.',
  'app.progress.scanTimelineTitle': 'ציר זמן הסריקות',
  'app.progress.openBeforeAfter': 'פתיחת לפני ואחרי',

  'app.beforeAfter.title': 'לפני ואחרי',
  'app.beforeAfter.body': 'השוו את תמונות נקודת הפתיחה מול נקודת בדיקה מאוחרת יותר. אותן ארבע זוויות, מיושרות באותו אופן.',
  'app.beforeAfter.compareLabel': 'להשוות מול',
  'app.beforeAfter.viewLabel': 'זווית',
  'app.beforeAfter.modeLabel': 'סגנון השוואה',
  'app.beforeAfter.mode.slider': 'מחוון',
  'app.beforeAfter.mode.sideBySide': 'זה לצד זה',
  'app.beforeAfter.mode.timeline': 'ציר זמן',
  'app.beforeAfter.reveal': 'חשיפת לפני או אחרי',
  'app.beforeAfter.missingBaseline': 'אין תמונת פתיחה לזווית זו.',
  'app.beforeAfter.missingCompare': 'אין עדיין תמונה לנקודת בדיקה וזווית אלה.',
  'app.beforeAfter.empty': 'הוסיפו תמונות מעקב כדי להשוות אותן מול נקודת הפתיחה.',
  'app.beforeAfter.addPhotos': 'הוספת תמונות מעקב',

  // --- לוח בקרה: תוצאות התוכנית (Track 2 WP-E) ---
  'app.results.eyebrow': 'תוצאות התוכנית',
  'app.results.title': 'תוצאות תוכנית ROOTÉ שלך',
  'app.results.notReady.title': 'דוח התוצאות שלך בהכנה',
  'app.results.notReady.body':
    'כאן ייפגשו הלפני והאחרי, הניתוח הראשוני והסופי וההתמדה שלך, לאחר שהתוכנית והסריקה הסופית יושלמו.',
  'app.results.notReady.cta': 'חזרה לסקירה',
  'app.results.summary.duration': 'משך התוכנית',
  'app.results.summary.dates': 'תאריכים',
  'app.results.summary.days': '{n} ימים',
  'app.results.beforeAfter.title': 'לפני ואחרי',
  'app.results.beforeAfter.body': 'תמונות נקודת הפתיחה שלך לצד הסריקה הסופית, בכל ארבע הזוויות.',
  'app.results.beforeAfter.cta': 'פתיחת ההשוואה המלאה',
  'app.results.beforeAfter.pendingFinal': 'התמונות הסופיות שלך יופיעו כאן לאחר השלמת הסריקה הסופית.',
  'app.results.analysis.title': 'ניתוח ראשוני וסופי',
  'app.results.analysis.initial': 'ראשוני',
  'app.results.analysis.final': 'סופי',
  'app.results.analysis.pending': 'השלימו את הסריקה הסופית כדי למלא את העמודה הסופית.',
  'app.results.adherence.title': 'התמדה',
  'app.results.adherence.body':
    'עד כמה השלמת בעקביות את השגרה היומית לאורך התוכנית. זהו מדד להשלמת שגרה, לא תוצאה קלינית.',
  'app.results.products.title': 'במה השתמשת',
  'app.results.timeline.title': 'ציר הזמן של התוכנית שלך',
  'app.results.next.title': 'הצעד הבא הוא בחירה שלך',
  'app.results.next.body':
    'ROOTÉ לא בוחרת עבורך צעד הבא. כשתהיו מוכנים, תוכלו להתחיל תוכנית חדשה, לבצע ניתוח שיער חדש, או להתייעץ עם צוות הליווי.',
  'app.results.next.startProgram': 'התחלת תוכנית חדשה',
  'app.results.next.newAnalysis': 'ביצוע ניתוח שיער חדש',
  'app.results.next.reviewNext': 'סקירת התוכנית הבאה שלי',
  'app.results.next.careTeam': 'הודעה לצוות הליווי',
  'app.results.actions.download': 'הורדת PDF',
  'app.results.actions.downloadHint': 'פותח את חלון ההדפסה של הדפדפן, לשמירה כ-PDF.',
  'app.results.actions.email': 'שליחת PDF במייל',
  'app.results.actions.emailStub': 'שליחת הדוח במייל תתחבר כששירות הדואר יופעל.',
  'app.results.pdfTitle': 'דוח תוצאות תוכנית ROOTÉ',

  // --- לוח בקרה: סקירת התוכנית הבאה (PO #19) ---
  'app.renew.eyebrow': 'התוכנית הבאה שלך',
  'app.renew.title': 'סקירת התוכנית הבאה שלי',
  'app.renew.body': 'התוכנית שלך הושלמה. פרופיל השיער שלך ממשיך איתך; בחרו כיצד להמשיך.',
  'app.renew.profileTitle': 'פרופיל השיער שלך עכשיו',
  'app.renew.profileNote': 'ממשיך מהסריקה האחרונה שלך. איכותני עד לחיבור שירות ניתוח.',
  'app.renew.chooseTitle': 'כיצד תרצו להמשיך?',
  'app.renew.path.continue.title': 'להמשיך באותה תוכנית',
  'app.renew.path.continue.body': 'להמשיך עם התוכנית שסיימת לתקופה נוספת.',
  'app.renew.path.maintain.title': 'מעבר לתחזוקה',
  'app.renew.path.maintain.body': 'שגרה קלה יותר לשמירה על מה שהשגת.',
  'app.renew.path.review.title': 'להעריך מחדש קודם',
  'app.renew.path.review.body': 'לבצע ניתוח שיער חדש לפני בחירת תוכנית.',
  'app.renew.reviewNote':
    'היכן שנדרש, רופא/ה או בית מרקחת בודקים את התוכנית שלך לפני אישורה. ROOTÉ אינה בוחרת עבורך צעד רפואי הבא.',
  'app.renew.cta.default': 'בחירת תוכנית',
  'app.renew.cta.review': 'התחלת ניתוח חדש',
  'app.renew.cta.later': 'לא עכשיו',

  // --- לוח בקרה: התראות ותזכורות (Track 2 WP-F; PO #21/#22) ---
  'app.nav.reminders': 'התראות ותזכורות',
  'app.reminders.title': 'התראות ותזכורות',
  'app.reminders.subtitle': 'על מה ROOTÉ תזכיר לך, ומתי.',
  'app.reminders.manage': 'ניהול התראות',
  'app.reminders.upcoming': 'בקרוב',
  'app.reminders.none': 'נקודות ציון קרובות, סריקות ותזכורות הזמנה חוזרת יופיעו כאן ככל שהתוכנית שלכם מתקדמת.',
  'app.reminders.settingsTitle': 'על מה להזכיר לי',
  'app.reminders.on': 'פעיל',
  'app.reminders.off': 'כבוי',
  'app.reminders.type.dailyTreatment': 'טיפול יומי',
  'app.reminders.type.shampooDay': 'יום חפיפה',
  'app.reminders.type.supplement': 'תוסף',
  'app.reminders.type.progressPhoto': 'תמונות מעקב',
  'app.reminders.type.progressScan': 'סריקת התקדמות',
  'app.reminders.type.finalScan': 'סריקה סופית',
  'app.reminders.type.reorder': 'הזמנה מחדש',
  'app.reminders.type.shipment': 'עדכוני משלוח',
  'app.reminders.type.programCompletion': 'סיום התוכנית',

  'app.care.title': 'צוות הליווי',
  'app.care.subtitle': 'הכוונה לאורך התוכנית, וקו ישיר כשצריך.',
  'app.care.dayTag': 'יום {day}',
  'app.care.msg.day1': 'ברוך הבא. מרח את התרחיף בבוקר ובערב, וצלם את סדרת התמונות הראשונה השבוע.',
  'app.care.msg.day14': 'עלייה קצרה בנשירה בסביבות זמן זה היא צפויה, כשהזקיקים מאפסים את המחזור. בדרך כלל זה מתייצב תוך כמה שבועות.',
  'app.care.msg.day45': 'הגיע הזמן לבדיקת תמונות. עקביות חשובה יותר מכל יום בודד, אז המשך.',
  'app.care.msg.day90': 'רוב האנשים יכולים להריץ ניתוח AI חדש בסביבות עכשיו כדי לראות איך הדפוס השתנה.',
  'app.care.msg.day180': 'חצי שנה בפנים. השווה את התמונות האחרונות שלך לתמונות הבסיס ובחן איתנו את התוכנית.',
  'app.care.compose.title': 'שליחת הודעה לצוות הליווי',
  'app.care.compose.send': 'שליחה',
  'app.care.compose.stub': 'תודה. צוות הליווי יגיב כאן באפליקציה (לא מחובר בתצוגה הזו).',
  'app.care.rescanLink': 'הרצת ניתוח שיער חדש',

  'app.rescan.title': 'ניתוח שיער חדש',
  'app.rescan.subtitle': 'סרוק מחדש כדי לראות איך השיער שלך השתנה ולעדכן את התוכנית.',
  'app.rescan.ready': 'אפשר להריץ ניתוח חדש עכשיו.',
  'app.rescan.locked': 'יהיה זמין בעוד {days} ימים, כשיהיה מספיק שינוי למדידה.',
  'app.rescan.cta': 'התחלת ניתוח חדש',
  'app.rescan.note': 'ניתוח חדש מפיק דוח מעודכן ועשוי להתאים את תוכנית הטיפול שלך.',
  'app.rescan.compare.title': 'בהשוואה לסריקת הבסיס',
  'app.rescan.compare.body': 'כשמבצעים סריקה חוזרת, התמונות החדשות מוצגות זו לצד זו עם הסריקה המקורית.',
  'app.rescan.compare.baseline': 'בסיס',
  'app.rescan.compare.latest': 'עדכני',
  'app.profile.title': 'הפרופיל שלך',
  'app.profile.account.title': 'חשבון',
  'app.profile.preferences.title': 'העדפות',
  'app.profile.chooseLanguage': 'בחירת שפה',
  'app.profile.email': 'אימייל',
  'app.profile.memberSince': 'חבר/ה מאז',
  'app.profile.program.title': 'התוכנית שלך',
  'app.profile.orderId': 'מספר הזמנה',
  'app.profile.duration': 'משך',
  'app.profile.dates': 'תאריכים',
  'app.profile.viewReport': 'צפייה בדוח שלי',
  'app.profile.orders.title': 'הזמנות אחרונות',
  'app.profile.orders.empty': 'היסטוריית ההזמנות שלכם תופיע כאן לאחר הרכישה הראשונה.',
  'app.profile.orders.program': 'תוכנית',
  'app.profile.orders.bag': 'חנות',
  'app.profile.password.title': 'שינוי סיסמה',
  'app.profile.password.currentLabel': 'סיסמה נוכחית',
  'app.profile.password.newLabel': 'סיסמה חדשה',
  'app.profile.password.submit': 'עדכון סיסמה',
  'app.profile.password.success': 'הסיסמה עודכנה.',
  'app.profile.password.error.wrong': 'הסיסמה הנוכחית שגויה.',
  'app.profile.password.error.weak': 'יש להשתמש בלפחות 8 תווים.',
  'app.profile.logout': 'התנתקות',
  'app.profile.leaveWarning.title': 'להתנתק כדי להמשיך?',
  'app.profile.leaveWarning.body': 'גלישה במוצרים תוציא אותך מהחשבון ותנתק אותך. תמיד אפשר להתחבר שוב.',
  'app.profile.leaveWarning.confirm': 'התנתקות והמשך',

  'hairScan.notConfigured': 'מתחברים ל-HairHealth.ai, נא לבדוק שוב בקרוב.',
  'hairScan.title': 'שוחחו עם HairHealth.ai',
  'hairScan.intro':
    'ענו על כמה שאלות קצרות ושתפו תמונה, והצוות של HairHealth.ai יבדוק את השיער שלכם ויעביר את התוצאה לצוות שלנו.',
  'hairScan.disclosure':
    'הצ׳אט הזה מופעל על ידי השותפה שלנו, HairHealth.ai. המידע שתשתפו כאן (התשובות והתמונות שלכם) נשלח אליה ומתווסף לרשימת אנשי הקשר שלנו כדי שהצוות שלנו יוכל ליצור איתכם קשר.',
  'hairScan.disclosureLink': 'קראו את מדיניות הפרטיות שלנו',

  'accountRescan.title': 'סריקה חוזרת עם HairHealth.ai',
  'accountRescan.intro':
    'בצעו סריקה חוזרת מהירה כדי שנוכל לעקוב אחר ההתקדמות שלכם. ענו על כמה שאלות ושתפו תמונה חדשה, והשותפה שלנו HairHealth.ai בוחנת אותה ומעבירה את התוצאה לצוות שלנו.',
  'accountRescan.disclosure':
    'הסריקה החוזרת הזו מופעלת על ידי השותפה שלנו, HairHealth.ai. המידע שתשתפו כאן נשלח אליה ומתווסף לרשימת אנשי הקשר שלנו, ומותאם לחשבון שלכם לפי כתובת האימייל.',
  'accountRescan.notConfigured': 'הסריקות החוזרות עדיין לא מחוברות, נא לבדוק שוב בקרוב.',
  'report.expect.outcome.m1': 'השגרה שלך מתבססת. ייתכנו נשירה זמנית מוגברת בזמן שמחזור השיער מתאפס.',
  'report.expect.outcome.m3': 'הנשירה בדרך כלל מתייצבת, ושינויים ראשוניים במרקם ובכיסוי עשויים להתחיל להופיע.',
  'report.expect.outcome.m6': 'שינויים נראים בצפיפות ובכיסוי מדווחים בדרך כלל עם המשך שימוש.',
  'app.preDelivery.mapCaption': 'מעקב המשלוח יופיע כאן ברגע שההזמנה שלך תצא לדרך.',
  'app.profile.orders.status': 'סטטוס',
  'app.profile.orders.statusConfirmed': 'ההזמנה אושרה',
  'app.profile.orders.payment': 'תשלום',
  'app.profile.orders.cardEnding': 'כרטיס שמסתיים ב-{last4}',
  'app.profile.orders.shipTo': 'משלוח אל',
  'app.profile.orders.programLength': 'משך התוכנית',
  'app.profile.orders.includes': 'מה כלול',
};
