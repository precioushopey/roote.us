...
export const PRODUCT_FAQS_COMMON: Faq[] = [
  ...
  },
  {
    id: 'sensitivity',
    q: L6({
      en: 'What if I have a sensitivity to an ingredient?',
      he: 'מה קורה אם יש לי רגישות לאחד הרכיבים?',
      ar: 'ماذا لو كانت لدي حساسية تجاه أحد المكونات؟',
      ru: 'Что делать, если у меня чувствительность к одному из компонентов?',
      fr: 'Que faire si je suis sensible à un ingrédient ?',
      es: '¿Qué pasa si tengo sensibilidad a algún ingrediente?',
    }),
    a: L6({
      en: 'Check the full ingredient list and the safety notes above before you start. If you have a known allergy or sensitivity, consult a healthcare professional before use.',
      he: 'יש לבדוק את רשימת הרכיבים המלאה ואת הערות הבטיחות שלמעלה לפני שמתחילים להשתמש. במקרה של אלרגיה או רגישות ידועה, יש להתייעץ עם איש מקצוע רפואי לפני השימוש.',
      ar: 'يُرجى مراجعة قائمة المكونات الكاملة وملاحظات السلامة أعلاه قبل البدء. في حال وجود حساسية أو تحسّس معروف، يُرجى استشارة أخصائي رعاية صحية قبل الاستخدام.',
      ru: 'Перед началом использования ознакомьтесь с полным списком ингредиентов и примечаниями по безопасности выше. При известной аллергии или чувствительности проконсультируйтесь с врачом перед применением.',
      fr: "Consultez la liste complète des ingrédients et les remarques de sécurité ci-dessus avant de commencer. En cas d'allergie ou de sensibilité connue, consultez un professionnel de santé avant utilisation.",
      es: 'Revise la lista completa de ingredientes y las notas de seguridad anteriores antes de empezar. Si tiene una alergia o sensibilidad conocida, consulte a un profesional de la salud antes de usarlo.',
    }),
  },
  {
    id: 'pause-or-stop',
    q: L6({
      en: 'What if I miss a day or want to pause?',
      he: 'מה קורה אם פספסתי יום או רוצה להשהות?',
      ar: 'ماذا لو فاتني يوم أو أردت إيقاف الاستخدام مؤقتًا؟',
      ru: 'Что делать, если я пропустил(а) день или хочу приостановить использование?',
      fr: "Que faire si j'oublie un jour ou que je veux faire une pause ?",
      es: '¿Qué pasa si me salto un día o quiero pausar?',
    }),
    a: L6({
      en: "Missing an occasional application isn't a problem, just pick your routine back up. To pause or stop a product in your program, manage that from your account rather than simply skipping it.",
      he: 'החמצה מזדמנת של שימוש אינה בעיה, פשוט לחזור לשגרה הרגילה. כדי להשהות או להפסיק מוצר בתוכנית, יש לנהל זאת דרך החשבון ולא רק להפסיק להשתמש.',
      ar: 'تفويت استخدام عرضي ليس مشكلة، يكفي العودة إلى الروتين المعتاد. لإيقاف منتج في برنامجك مؤقتًا أو نهائيًا، يُدار ذلك من حسابك وليس فقط بالتوقف عن استخدامه.',
      ru: 'Пропуск отдельного применения не проблема, просто вернитесь к обычному режиму. Чтобы приостановить или прекратить использование продукта в вашей программе, управляйте этим через свой аккаунт, а не просто прекращайте использование.',
      fr: "Manquer une application occasionnelle n'est pas un problème, reprenez simplement votre routine habituelle. Pour mettre en pause ou arrêter un produit de votre programme, gérez cela depuis votre compte plutôt que de simplement arrêter de l'utiliser.",
      es: 'Saltarse una aplicación ocasional no es un problema: simplemente retome su rutina habitual. Para pausar o detener un producto de su programa, gestiónelo desde su cuenta en lugar de simplemente dejar de usarlo.',
    }),
  },
];
